...
class FromList extends QueryTreeNodeVector<ResultSetNode> implements
		OptimizableList {
	...
	// FromList could have a view in it's list. If the view is defined in
	// SESSION
	...
	/*
	 * Whether or not this FromList is transparent. A "transparent" FromList is
	 * one in which all FromTables are bound based on an outer query's FromList.
	 * This means that the FromTables in the transparent list are allowed to see
	 * and reference FromTables in the outer query's list. Or put differently, a
	 * FromTable which sits in a transparent FromList does not "see" the
	 * transparent FromList when binding; rather, it sees (and can therefore
	 * reference) the FromList of an outer query.
	 */
	 ...
	 *
	 * @param optimizeJoinOrder
	 *            {@code true} if join order optimization is to be performed
	 * @param cm
	 *            context manager
	 */
	 ...
	 *            {@code true} if join order optimization is to be performed
	 * @param fromTable
	 *            initialize list with this table
	 * @param cm
	 *            context manager
	 * @exception StandardException
	 *                Thrown on error
	 */
	FromList(boolean optimizeJoinOrder, FromTable fromTable, ContextManager cm)
	...
	final void addFromTable(FromTable fromTable) throws StandardException {
		/*
		 * Don't worry about checking TableOperatorNodes since they don't have
		 * exposed names. This will potentially allow duplicate exposed names in
		 * some degenerate cases, but the binding of the ColumnReferences will
		 * catch those cases with a different error. If the query does not have
		 * any ColumnReferences from the duplicate exposed name, the user is
		 * executing a really dumb query and we won't throw and exception -
		 * consider it an ANSI extension.
		 */
		TableName leftTable;
			...
			for (int index = 0; index < size; index++) {
				...
				if (leftTable.equals(rightTable)) {
					throw StandardException.newException(
							SQLState.LANG_FROM_LIST_DUPLICATE_TABLE_NAME,
	...
	public boolean referencesSessionSchema() throws StandardException {
		...
		// definition. Hence, we need to intercept in the middle on the
		// bindTables
	...
	/**
	 * Go through the list of the tables and see if the passed ResultColumn is a
	 * join column for a right outer join with USING/NATURAL clause.
	 * 
	...
	void bindTables(DataDictionary dataDictionary, FromList fromListParam)
		...
		boolean wasSkippingTypePrivileges = getCompilerContext()
				.skipTypePrivileges(true);

		FromTable fromTable;

		/*
		 * Now we bind the tables - this is a 2 step process. We first bind all
		 * of the non-VTIs, then we bind the VTIs. This enables us to handle the
		 * passing of correlation columns in VTI parameters. NOTE: We set the
		 * table numbers for all of the VTIs in the first step, when we find
		 * them, in order to avoid an ordering problem with join columns in
		 * parameters.
		 */
		int size = size();
		for (int index = 0; index < size; index++) {
			fromTable = (FromTable) elementAt(index);
			FromTable newNode = (FromTable) fromTable.bindNonVTITables(
					dataDictionary, fromListParam);
			// If the fromTable is a view in the SESSION schema, then we need to
			// save that information
			// in referencesSessionSchema element. The reason for this is that
			// the view will get
			// replaced by it's view definition and we will loose the
			// information that the statement
		...
		for (int index = 0; index < size; index++) {
			fromTable = (FromTable) elementAt(index);
			FromTable newNode = (FromTable) fromTable
					.bindVTITables(fromListParam);
			if (fromTable.referencesSessionSchema()) {
				referencesSessionSchema = true;
			}
			newNode.setMergeTableID(fromTable.getMergeTableID());
		...
		for (int index = 0; index < size; index++) {
			...
			if (fromTable.isPrivilegeCollectionRequired()
				...
				cc.addRequiredColumnPriv(fromTable.getTableDescriptor()
						.getColumnDescriptor(1));
			}
		}
	...
	/**
	 * Bind the result columns of the ResultSetNodes in this FromList when there
	 * is no base table to bind them to. This is useful for SELECT statements,
	 * where the result columns get their types from the expressions that live
	 * under them.
	 *
	 * @param fromListParam
	...
	/**
	 * Expand a "*" into the appropriate ResultColumnList. If the "*" is
	 * unqualified it will expand into a list of all columns in all of the base
	 * tables in the from list at the current nesting level; otherwise it will
	 * expand into a list of all of the columns in the base table that matches
	 * the qualification.
	 *
	 * NOTE: Callers are responsible for ordering the FromList by nesting level,
	 * with tables at the deepest (current) nesting level first. We will expand
	 * the "*" into a list of all columns from all tables having the same
	 * nesting level as the first FromTable in this list. The check for nesting
	 * level exists because it's possible that this FromList includes FromTables
	 * from an outer query, which can happen if there is a "transparent"
	 * FromList above this one in the query tree. Ex:
	 *
	 * select j from onerow where exists (select 1 from somerow union select *
	 * from diffrow where onerow.j < diffrow.k)
	 *
	 * If "this" is the FromList for the right child of the UNION then it will
	 * contain both "diffrow" and "onerow", the latter of which was passed down
	 * via a transparent FromList (to allow binding of the WHERE clause). In
	 * that case the "*" should only expand the result columns of "diffrow"; it
	 * should not expand the result columns of "onerow" because that table is
	 * from an outer query. We can achieve this selective expansion by looking
	 * at nesting levels.
	 * 
	 * @param allTableName
	...
	ResultColumnList expandAll(TableName allTableName) throws StandardException {
		 ...
		 * current nesting level if the "*" is not qualified (allTableName is
		 * null). Current nesting level is determined by the nesting level of
		 * the first FromTable in the list.
		 */
		int targetNestingLevel = ((FromTable) elementAt(0)).getLevel();
		int size = size();

		/*
		 * Make sure our assumption about nesting-based ordering has been
		 * satisified. I.e. that the list is ordered with the most deeply nested
		 * FromTables first.
		 */
		...
		for (int index = 0; index < size; index++) {
			...
			if (targetNestingLevel != fromTable.getLevel()) {
				/*
				 * We only expand result columns for tables at the target
				 * nesting level. Since the FromTables are sorted based on
				 * nesting level, we're done if we get here.
				 */
				break;
			}

			/*
			 * We let the FromTable decide if there is a match on the exposed
			 * name. (A JoinNode will not have an exposed name, so it will need
			 * to pass the info to its left and right children.)
			 */
			...
			/*
			 * Expand the column list and append to the list that we will
			 * return.
			 */
			...
			/*
			 * If the "*" is qualified, then we can stop the expansion as soon
			 * as we find the matching table.
			 */
		...
		if (resultColumnList == null) {
			throw StandardException.newException(
					SQLState.LANG_EXPOSED_NAME_NOT_FOUND, allTableName);
		}
	 ...
	 * name must be unique within the tables in the FromList. An exception is
	 * thrown if a column name is not unique. This method fills in various
	 * fields in the column reference.
	 * </p>
	 *
	 * <p>
	 * NOTE: Callers are responsible for ordering the FromList by nesting level,
	 ...
	 * walking the list when the nesting level changes and we have found a
	 * match.
	 * </p>
	 *
	 * <p>
	 * NOTE: If the ColumnReference is qualified, then we will stop the search
	 * at the first nesting level where there is a match on the exposed table
	 * name. For example,
	 * <p>
	 *
	 * <pre>
	...
	ResultColumn bindColumnReference(ColumnReference columnReference)
		...
		/*
		 * * Find the first table with matching column name. If there* is more
		 * than one table with a matching column name at the same* nesting
		 * level, give an error.
		 */
		int size = size();
		for (int index = 0; index < size; index++) {
			...
			//
			// If the MERGE statement has marked its variables, we expect to
			// follow
			// its judgment. Make sure that we only match SOURCE columns to
			// SOURCE tables and TARGET columns to TARGET tables.
			//
			if ((fromTable.getMergeTableID() != ColumnReference.MERGE_UNKNOWN)
					&& (columnReference.getMergeTableID() != ColumnReference.MERGE_UNKNOWN)
					&& (fromTable.getMergeTableID() != columnReference
							.getMergeTableID())) {
				continue;
			}

			/*
			 * We can stop if we've found a matching column or table name at the
			...
			if (resultColumn != null) {
				if (!columnNameMatch) {
					 ...
					 * table number/column number from the underlying table, not
					 * the join node. This is important for beging able to push
					 * predicates down through join nodes.
					 */
					matchingRC = resultColumn;
					columnReference.setSource(resultColumn);
					/*
					 * Set the nesting level at which the CR appears and the
					 * nesting level of its source RC.
					 */
					columnReference.setNestingLevel(((FromTable) elementAt(0))
							.getLevel());
					...
					if (fromTable.isPrivilegeCollectionRequired()) {
						getCompilerContext().addRequiredColumnPriv(
								resultColumn.getTableColumnDescriptor());
					}
			...
			/*
			 * Remember if we get a match on the exposed table name, so that we
			 * can stop at the beginning of the next level.
			 */
			tableNameMatch = tableNameMatch
					|| (crTableName != null && crTableName.equals(fromTable
							.getExposedName()));
		}

		// fill in the table name
		if ((matchingTable != null) && (matchingRC != null)
				&& (columnReference.getTableName() == null)) {
	...
	/**
	 * Preprocess the query tree - this currently means: o Generating a
	 ...
	 * and HAVING clauses into PredicateLists and classifying them. o Flatten
	 * those FromSubqueries which can be flattened. o Ensuring that a
	 * ProjectRestrictNode is generated on top of every FromBaseTable and
	 * generated in place of every FromSubquery which could not be flattened. o
	 * Pushing single table predicates down to the new ProjectRestrictNodes.
	 *
	 * @param numTables
	...
	/**
	 * Flatten all the FromTables that are flattenable. RESOLVE - right now we
	 * just flatten FromSubqueries. We should also flatten flattenable JoinNodes
	 * here.
	 *
	 * @param rcl
	 *            The RCL from the outer query
	 * @param predicateList
	 *            The PredicateList from the outer query
	 * @param sql
	 ...
	 *                Thrown on error
	 */
	void flattenFromTables(ResultColumnList rcl, PredicateList predicateList,
		...
		/*
		 * Loop until all flattenable entries are flattened. We restart the
		 * inner loop after flattening an in place to simplify the logic and so
		 * that we don't have to worry about walking a list while we are
		 * modifying it.
		 */
		while (flattened) {
			flattened = false;
			...
			for (int index = 0; index < size() && !flattened; index++) {
				...
				if ((ft instanceof FromSubquery) || ft.isFlattenableJoinNode()) {
					...
					flattenedTableNumbers.add(ReuseFactory.getInteger(ft
							.getTableNumber()));

					/*
					 * Remove the node from the list and insert its FromList
					 * here.
					 */
					FromList flatteningFL = ft.flatten(rcl, predicateList, sql,
							gbl, havingClause);
					if (SanityManager.DEBUG) {
					...
					if (flatteningFL != null) {
						...
						for (int inner = 1; inner < innerSize; inner++) {
							insertElementAt(flatteningFL.elementAt(inner),
									index + inner);
						}
					} else {
						/*
						 * * If flatten returns null, that means it wants to* be
						 * removed from the FromList.
						 */
		...
		if (!flattenedTableNumbers.isEmpty()) {
			for (int i = 0; i < size(); i++) {
				...
				if (ft instanceof ProjectRestrictNode) {
					ResultSetNode rst = ((ProjectRestrictNode) ft)
							.getChildResult();
					if (rst instanceof FromBaseTable) {
						((FromBaseTable) rst)
								.clearDependency(flattenedTableNumbers);
					}
				}
			}
		}
	...
	void pushPredicates(PredicateList predicateList) throws StandardException {
		 ...
		 * NOTE: The PredicateList may be empty, but that's okay, we still call
		 * pushExpressions() for each entry in the FromList because that's where
		 * any outer join conditions will get pushed down.
		 */
	...
	void setProperties(Properties props) throws StandardException {
		...
		/*
		 * * Validate the properties list now. This is possible because* there
		 * is nothing in this properties list that relies on binding* or
		 * optimization to validate.
		 */
		...
		while (e.hasMoreElements()) {
			...
			if (key.equals("joinOrder")) {
				if (StringUtil.SQLEqualsIgnoreCase(value, "fixed")) {
					...
					throw StandardException.newException(
							SQLState.LANG_INVALID_JOIN_ORDER_SPEC, value);
				}
				...
				if (StringUtil.SQLEqualsIgnoreCase(value, "true")) {
					...
					throw StandardException.newException(
							SQLState.LANG_INVALID_STATISTICS_SPEC, value);
				}
			} else {
				throw StandardException.newException(
						SQLState.LANG_INVALID_FROM_LIST_PROPERTY, key, value);
			}
		}
	}
	...
	public void reOrder(int[] joinOrder) {
		...
		if (SanityManager.DEBUG) {
			if (joinOrder.length != size()) {
				SanityManager.THROWASSERT("In reOrder(), size of FromList is "
						+ size() + " while size of joinOrder array is "
						+ joinOrder.length);
			}

			/*
			 * * Determine that the values in the list are unique and in range.*
			 * The easiest way to determine that they are unique is to add* them
			 * all up and see whether the result is what's expected* for that
			 * array size.
			 */
			int sum = 0;
			...
			if (sum != ((joinOrder.length * (joinOrder.length - 1)) / 2)) {
				...
				SanityManager
						.THROWASSERT("joinOrder array has some duplicate value: "
								+ arrayVals);
			}
		}
	 ...
	 * from list. This is useful when flattening a subquery.
	 *
	 * @param decrement
	...
	void decrementLevel(int decrement) {
		...
		for (int index = 0; index < size; index++) {
			...
			/*
			 * Decrement the level of any CRs in single table predicates that
			 * are interesting to transitive closure.
			 */
	...
	/**
	 * This method is used for both subquery flattening and distinct elimination
	 * based on a uniqueness condition. For subquery flattening we want to make
	 * sure that the query block will return at most 1 row. For distinct
	 * elimination we want to make sure that the query block will not return any
	 * duplicates. This is true if every table in the from list is (a base table
	 * and the set of columns from the table that are in equality comparisons
	 * with expressions that do not include columns from the same table is a
	 * superset of any unique index on the table) or an EXISTS FBT. In addition,
	 * at least 1 of the tables in the list has a set of columns in equality
	 * comparisons with expressions that do not include column references from
	 * the same query block is a superset of a unique index on that table. (This
	 * ensures that the query block will onlyr return a single row.) This method
	 * is expected to be called after normalization and after the from list has
	 * been preprocessed. It can be called both before and after the predicates
	 * have been pulled from the where clause. The algorithm for this is as
	 * follows
	 *
	 * If any table in the query block is not a base table, give up. For each
	 * table in the query Ignore exists table since they can only produce one
	 * row
	 *
	 * create a matrix of tables and columns from the table (tableColMap) (this
	 * is used to keep track of the join columns and constants that can be used
	 * to figure out whether the rows from a join or in a select list are
	 * distinct based on unique indexes)
	 *
	 * create an array of columns from the table(eqOuterCol) (this is used to
	 * determine that only one row will be returned from a join)
	 *
	 * if the current table is the table for the result columns set the result
	 * columns in the eqOuterCol and tableColMap (if these columns are a
	 * superset of a unique index and all joining tables result in only one row,
	 * the results will be distinct) go through all the predicates and update
	 * tableColMap and eqOuterCol with join columns and correlation variables,
	 * parameters and constants since setting constants, correlation variables
	 * and parameters, reduces the number of columns required for uniqueness in
	 * a multi-column index, they are set for all the tables (if the table is
	 * not the result table, in this case only the column of the result table is
	 * set) join columns are just updated for the column in the row of the
	 * joining table.
	 *
	 * check if the marked columns in tableColMap are a superset of a unique
	 * index (This means that the join will only produce 1 row when joined with
	 * 1 row of another table) check that there is a least one table for which
	 * the columns in eqOuterCol(i.e. constant values) are a superset of a
	 * unique index (This quarantees that there will be only one row selected
	 * from this table).
	 *
	 ...
	 *
	 * @param rcl
	 *            If non-null, the RCL from the query block. If non-null for
	 *            subqueries, then entry can be considered as part of an =
	 *            comparison.
	 * @param whereClause
	 *            The WHERE clause to consider.
	 * @param wherePredicates
	 *            The predicates that have already been pulled from the WHERE
	 *            clause.
	 * @param dd
	 *            The DataDictionary to use.
	 *
	 * @return Whether or not query block will return at most 1 row for a
	 *         subquery, no duplicates for a distinct.
	 *
	 * @exception StandardException
	 *                Thrown on error
	 */
	boolean returnsAtMostSingleRow(ResultColumnList rcl, ValueNode whereClause,
			PredicateList wherePredicates, DataDictionary dd)
			throws StandardException {
		...
		/*
		 * When considering subquery flattening, we are interested in the 1st
		 * (and only) entry in the RCL. (The RCL will be null if result column
		 * is not of interest for subquery flattening.) We are interested in all
		 * entries in the RCL for distinct elimination.
		 */
		if (rcl != null) {
		...
		/*
		 * First see if all entries are FromBaseTables. No point in continuing
		 * if not.
		 */
		int size = size();
		for (int index = 0; index < size; index++) {
			...
			// Following for loop code is to take care of Derby-251 (DISTINCT
			// returns
			// duplicate rows).
			// Derby-251 returned duplicate rows because we were looking at
			// predicates
			// that belong to existsTable to determine DISTINCT elimination
			//
			// (Check method level comments to understand DISTINCT elimination
			// rules.)
			//
			...
			// (select q3."NO1" from IDEPT q3 where (q3."REPORTTO_NO" =
			// q2."NO1")))
			// (select q3."NO1" from IDEPT q3 where ( ABS(q3."REPORTTO_NO") =
			// q2."NO1")))
			//
			// Table IDEPT in the query above has a primary key defined on
			// column "NO1"
			...
			// ) ;
			//
			// For the optimized query above, Derby generates following
			// predicates.
			// ABS(q3.reportto_no) = q2.no1
			// q2.discrim_dept = 'HardwareDept'
			// q1.descrim_dept = 'SoftwareDept'
			// q1.no1 = q3.no1
			// The predicate ABS(q3."NO1") = q1."NO1" should not be considered
			// when trying
			...
			// equality comparisons. To achieve this, predicates based on exists
			// base
			// table q3 (the first and the last predicate) should be removed
			// while
			...
			if (fbt.getExistsBaseTable()) {
				...
				for (int i = predicatesTemp.size() - 1; i >= 0; i--) {
					AndNode topAndNode = predicatesTemp.elementAt(i)
							.getAndNode();

					for (ValueNode whereWalker = topAndNode; whereWalker instanceof AndNode; whereWalker = ((AndNode) whereWalker)
							.getRightOperand()) {
						// See if this is a candidate =
						AndNode and = (AndNode) whereWalker;

						// we only need to worry about equality predicates
						// because only those
						// predicates are considered during DISTINCT
						// elimination.
						if (!and.getLeftOperand().isRelationalOperator()
								|| !(((RelationalOperator) (and
										.getLeftOperand())).getOperator() == RelationalOperator.EQUALS_RELOP)) {
							continue;
						}

						JBitSet referencedTables = and.getLeftOperand()
								.getTablesReferenced();
		...
		/*
		 * Build an array of tableNumbers from this query block. We will use
		 * that array to find out if we have at least one table with a
		 * uniqueness condition based only on constants, parameters and
		 * correlation columns.
		 */
		...
		for (int index = 0; index < size; index++) {
			...
			/*
			 * Now see if there are any equality conditions of interest in the
			 * where clause.
			 */
			if (whereClause != null) {
				whereClause.checkTopPredicatesForEqualsConditions(tableNumber,
						eqOuterCols, tableNumbers, tableColMap[index],
						resultColTable);
			}
			 ...
			 * where predicates.
			 */
			predicatesTemp.checkTopPredicatesForEqualsConditions(tableNumber,
					eqOuterCols, tableNumbers, tableColMap[index],
					resultColTable);

			/*
			 * Now see if there are any equality conditions of interest that
			 * were already pushed down to the PRN above the FBT. (Single table
			 * predicates.)
			 */
			...
			/*
			 * We can finally check to see if the marked columns are a superset
			 * of any unique index.
			 */
			...
			/*
			 * Do we have at least 1 table whose equality condition is based
			 * solely on constants, parameters and correlation columns.
			 */
		...
		if (satisfiesOuter) {
			/*
			 * check that all the tables are joined by unique indexes or only
			 * produce 1 row
			 */
	...
	/**
	 * Mark all of the FromBaseTables in the list as EXISTS FBTs. Each EBT has
	 * the same dependency list - those tables that are referenced minus the
	 * tables in the from list.
	 *
	 * @param referencedTableMap
	 *            The referenced table map.
	 * @param outerFromList
	...
	void genExistsBaseTables(JBitSet referencedTableMap,
			FromList outerFromList, boolean isNotExists)
			throws StandardException {
		...
		if (SanityManager.DEBUG) {
			if (size() != 1) {
				SanityManager.THROWASSERT("size() expected to be 1, not "
						+ size());
			}
		}
		...
		int size = size();
		for (int index = 0; index < size; index++) {
			ResultSetNode ft = ((ProjectRestrictNode) elementAt(index))
					.getChildResult();
		...
		/*
		 * Degenerate case - If flattening a non-correlated EXISTS subquery then
		 * we need to make the table that is getting flattened dependendent on
		 ...
		 * The reason that duplicates can be introduced is that we do special
		 * processing in the join to make sure only one qualified row from the
		 * right side is returned. If the exists table is on the left, we can
		 * return all the qualified rows.
		 */
		if (dependencyMap.getFirstSetBit() == -1) {
			int outerSize = outerFromList.size();
			for (int outer = 0; outer < outerSize; outer++)
				dependencyMap.or(((FromTable) outerFromList.elementAt(outer))
		...
		for (int index = 0; index < size; index++) {
			...
			if (fromTable instanceof ProjectRestrictNode) {
				...
				if (prn.getChildResult() instanceof FromBaseTable) {
					FromBaseTable fbt = (FromBaseTable) prn.getChildResult();
					fbt.setExistsBaseTable(true,
							(JBitSet) dependencyMap.clone(), isNotExists);
				}
			}
		}
	}
	 ...
	 *
	 * This routine searches for the indicated table number in the fromlist and
	 * returns TRUE if the table is present in the from list and is marked NOT
	 * EXISTS, false otherwise.
	 *
	 * A table may be present in the from list for NOT EXISTS if it is used as a
	 * correlated NOT EXISTS subquery. In such a situation, when the subquery is
	 * flattened, it is important that we remember that this is a NOT EXISTS
	 * subquery, because the join semantics are different (we're looking for
	 * rows that do NOT match, rather than rows that do). And since the join
	 * semantics are different, we cannot include this table into a transitive
	 * closure of equijoins (See DERBY-3033 for a situation where this occurs).
	 *
	 * @param tableNumber
	...
	int updateTargetLockMode() {
		if (SanityManager.DEBUG) {
			if (size() != 1) {
				SanityManager.THROWASSERT("size() is " + size()
						+ " but should be 1");
			}
		}
	...
	/**
	 * Indicate that this FromList is "transparent", which means that its
	 * FromTables should be bound to tables from an outer query. Generally this
	 * is not allowed, but there are exceptions. See
