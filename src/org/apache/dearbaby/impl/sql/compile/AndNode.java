...
class AndNode extends BinaryLogicalOperatorNode {
	 ...
	 */
	AndNode(ValueNode leftOperand, ValueNode rightOperand, ContextManager cm)
			throws StandardException {
	 ...
	 * @throws StandardException
	 */
	AndNode(ValueNode leftOperand, ValueNode rightOperand, String methodName,
	...
	/**
	 * Bind this logical operator. All that has to be done for binding a logical
	 * operator is to bind the operands, check that both operands are
	 * BooleanDataValue, and set the result type to BooleanDataValue.
	 *
	 * @param fromList
	...
	ValueNode preprocess(int numTables, FromList outerFromList,
			...
			throws StandardException {
		/*
		 * If the left child is an OR, then mark it as the 1st OR in the list.
		 * That will allow us to consider converting the OR to an IN list when
		 * we preprocess the 1st OR in the list.
		 */
		...
		/*
		 * We need to rerun the changeToCNF() phase if our left operand is an
		 * AndNode. This can happen due to a predicate transformation, such as
		 * the ones for LIKE and BETWEEN, underneath us.
		 */
	...
	public void genQuery0() {
		...
		leftOperand.genCondition();
		rightOperand.genCondition();
	 ...
	 * AndNodes terminated by a true BooleanConstantNode.
	 *
	 * @return The modified expression
	 *
	 * @exception StandardException
	...
	/**
	 * Verify that putAndsOnTop() did its job correctly. Verify that the top
	 * level of the expression is a chain of AndNodes terminated by a true
	 * BooleanConstantNode.
	 *
	...
	boolean verifyPutAndsOnTop() {
		...
		if (SanityManager.ASSERT) {
			isValid = ((rightOperand instanceof AndNode) || (rightOperand
					.isBooleanTrue()));
	...
	ValueNode changeToCNF(boolean underTopAndNode) throws StandardException {
		 ...
		 * (putAndsOnTop() has taken care of this. If the last node in the chain
		 * is not a true BooleanConstantNode then we need to do the
		...
		if (!(rightOperand instanceof AndNode)
			...
			BooleanConstantNode trueNode = new BooleanConstantNode(true,
					getContextManager());
			AndNode newRightOperand = new AndNode(curAnd.getRightOperand(),
		 ...
		 * 
		 * this / \ And2 Nodex / \ ... left2 right2
		 * 
		 * to:
		 * 
		 * this / \ left2.changeToCNF() And2 / \ right2.changeToCNF()
		 * Nodex.changeToCNF()
		 * 
	...
	/**
	 * Verify that changeToCNF() did its job correctly. Verify that: o AndNode -
	 * rightOperand is not instanceof OrNode leftOperand is not instanceof
	 * AndNode o OrNode - rightOperand is not instanceof AndNode leftOperand is
	 * not instanceof OrNode
	 *
	 * @return Boolean which reflects validity of the tree.
	 */
	@Override
	...
	/**
	 * Do bind() by hand for an AndNode that was generated after bind(), eg by
	 * putAndsOnTop(). (Set the data type and nullability info.)
	 *
	 * @exception StandardException
	 *                Thrown on error
	 */
	void postBindFixup() throws StandardException {
		setType(resolveLogicalBinaryOperator(leftOperand.getTypeServices(),
