...
public abstract class BinaryComparisonOperatorNode extends BinaryOperatorNode {
	 ...
	 * @param methodName
	 *            The name of the method to call in the generated class
	 * @param forQueryRewrite
	 *            Can be true only if if this node has been added by an internal
	 *            rewrite of the query. This allows binding to be more liberal
	 *            when checking it against allowed syntax. This parameter will
	 *            be passed FALSE when a new instance of the node is being
	 *            created(which is the majority of the cases). But when an
	 *            existing node is getting cloned, the value of this parameter
	 *            should be passed as the originalNode.getForQueryRewrite().
	 *            Examples of this can be found in Predicate.Java and
	 *            PredicateList.java
	 * @param cm
	 *            The context manager
	 */

	BinaryComparisonOperatorNode(ValueNode leftOperand, ValueNode rightOperand,
			String operator, String methodName, boolean forQueryRewrite,
			ContextManager cm) throws StandardException {
		super(leftOperand, rightOperand, operator, methodName,
	...
	/**
	 * This node was generated as part of a query rewrite. Bypass the normal
	...
	/**
	 * Bind this comparison operator. All that has to be done for binding a
	 * comparison operator is to bind the operands, check the compatibility of
	 * the types, and set the result type to SQLBoolean.
	 *
	 * @param fromList
	...
	ValueNode bindExpression(FromList fromList, SubqueryList subqueryList,
		...
		/*
		 * If we are comparing a non-string with a string type, then we must
		 * prevent the non-string value from being used to probe into an index
		 * on a string column. This is because the string types are all of low
		 * precedence, so the comparison rules of the non-string value are used,
		 * so it may not find values in a string index because it will be in the
		 * wrong order. So, cast the string value to its own type. This is
		 * easier than casting it to the non-string type, because we would have
		 * to figure out the right length to cast it to.
		 */

		DataTypeDescriptor rightTypeServices = rightOperand.getTypeServices();

		rightOperand = new CastNode(rightOperand, new DataTypeDescriptor(
				TypeId.BOOLEAN_ID, true, 0), getContextManager());
		((CastNode) rightOperand).bindCastNodeOnly();

		DataTypeDescriptor leftTypeServices = leftOperand.getTypeServices();

		leftOperand = new CastNode(leftOperand, new DataTypeDescriptor(
				TypeId.BOOLEAN_ID, true, 0), getContextManager());
	...
	/**
	 * Test the type compatability of the operands and set the type info for
	 * this node. This method is useful both during binding and when we generate
	 * nodes within the language module outside of the parser.
	 *
	 * @exception StandardException
	 *                Thrown on error
	 */
	...
	ValueNode preprocess(int numTables, FromList outerFromList,
		...
		/*
		 * This is where we start to consider flattening expression subqueries
		 * based on a uniqueness condition. If the right child is a SubqueryNode
		 * then it is a potentially flattenable expression subquery. If we
		 * flatten the subquery then we at least need to change the right
		 * operand of this comparison. However, we may want to push the
		 * comparison into the subquery itself and replace this outer comparison
		 * with TRUE in the tree. Thus we return rightOperand.preprocess() if
		 * the rightOperand is a SubqueryNode. NOTE: SubqueryNode.preprocess()
		 * is smart enough to return this node if it is not flattenable. NOTE:
		 * We only do this if the subquery has not yet been preprocessed. (A
		 * subquery can get preprocessed multiple times if it is a child node in
		 * an expression that gets transformed, like BETWEEN. The subquery
		 * remembers whether or not it has been preprocessed and simply returns
		 * if it has already been preprocessed. The return returns the
		 * SubqueryNode, so an invalid tree is returned if we set the parent
		 * comparison operator when the subquery has already been preprocessed.)
		 */
		if ((rightOperand instanceof SubqueryNode)
			...
			return rightOperand.preprocess(numTables, outerFromList,
					outerSubqueryList, outerPredicateList);
		} else {
			rightOperand = rightOperand.preprocess(numTables, outerFromList,
					outerSubqueryList, outerPredicateList);
			return this;
		}
	}
	 ...
	 * operands swapped. The node type may also be changed if the operator is
	 * not symmetric.
	 * </p>
	 *
	 * <p>
	 * This method may for instance be used to normalize a predicate by moving
	 * constants to the right-hand side of the comparison. Example:
	...
	ValueNode changeToCNF(boolean underTopAndNode) throws StandardException {
		/*
		 * If our right child is a subquery and we are under a top and node then
		 * we want to mark the subquery as under a top and node. That will allow
		 * us to consider flattening it.
		 */
