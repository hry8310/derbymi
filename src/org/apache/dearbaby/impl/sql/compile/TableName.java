...
/**
 * A TableName represents a qualified name, externally represented as a schema
 * name and an object name separated by a dot. This class is misnamed: it is
 * used to represent the names of other object types in addition to tables.
 *
 */

public class TableName extends QueryTreeNode {
	/*
	 * Both schemaName and tableName can be null, however, if* tableName is null
	 * then schemaName must also be null.
	 */
	 ...
	 *
	 * @param schemaName
	 *            The name of the schema being referenced
	 * @param tableName
	 *            The name of the table or other object being referenced
	 * @param cm
	 *            The context manager
	 */
	 ...
	 *            begin position of token for the table name identifier from
	 *            parser. pass in -1 if unknown
	 * @param tokEndOffset
	 *            end position of token for the table name identifier from
	 *            parser. pass in -1 if unknown
	 * @param cm
	 *            The context manager
	 */
	TableName(String schemaName, String tableName, int tokBeginOffset,
	 ...
	 * SELECT * is expanded). Also, only check table names if the schema name(s)
	 * are null.
	 *
	...
	boolean equals(String otherSchemaName, String otherTableName) {
		...
		if (fullTableName == null) {
			return true;
		} else if ((schemaName == null) || (otherSchemaName == null)) {
	...
	/**
	 * Bind this TableName. This means filling in the schema name if it wasn't
	 * specified.
	 *
