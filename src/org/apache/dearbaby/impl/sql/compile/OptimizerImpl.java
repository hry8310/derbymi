...
class OptimizerImpl implements Optimizer {
	...
	public OptimizerImpl(OptimizableList optimizableList,
		...
		bestCost = getNewCostEstimate(Double.MAX_VALUE, Double.MAX_VALUE,
				Double.MAX_VALUE);
	...
	public void prepForNextRound() {
		...
		/*
		 * Since we're preparing for a new round, we have to clear out the
		 * "bestCost" from the previous round to ensure that, when this round of
		 * optimizing is done, bestCost will hold the best cost estimate found
		 * _this_ round, if there was one. If there was no best cost found
		 * (which can happen if there is no feasible join order) then bestCost
		 * will remain at Double.MAX_VALUE. Then when outer queries check the
		 * cost and see that it is so high, they will reject whatever outer join
		 * order they're trying in favor of something that's actually valid (and
		 * therefore cheaper).
		 * 
		 * Note that we do _not_ reset the "foundABestPlan" variable nor the
		 * "bestJoinOrder" array. This is because it's possible that a
		 * "best join order" may not exist for the current round, in which case
		 * this OptimizerImpl must know whether or not it found a best join
		 * order in a previous round (foundABestPlan) and if so what the
		 * corresponding join order was (bestJoinOrder). This information is
		 * required so that the correct query plan can be generated after
		 * optimization is complete, even if that best plan was not found in the
		 * most recent round.
		 */
		bestCost = getNewCostEstimate(Double.MAX_VALUE, Double.MAX_VALUE,
				Double.MAX_VALUE);
		 ...
		 * the next round of optimization. Otherwise if we timed out during a
		 * previous round and then we get here for another round, we'll
		 * immediately "timeout" again before optimizing any of the Optimizables
		 * in our list. This is okay if we don't have any predicates from outer
		 * queries because in that case the plan we find this round will be the
		 * same one we found in the previous round, in which case there's no
		 * point in resetting the timeout state and doing the work a second
		 * time. But if we have predicates from an outer query, those predicates
		 * could help us find a much better plan this round than we did in
		 * previous rounds--so we reset the timeout state to ensure that we have
		 * a chance to consider plans that can take advantage of the pushed
		 * predicates.
		 */
		usingPredsPushedFromAbove = false;
		if ((predicateList != null) && (predicateList.size() > 0)) {
			for (int i = predicateList.size() - 1; i >= 0; i--) {
				...
				if (((Predicate) predicateList.getOptPredicate(i))
						.isScopedForPush()) {
		...
		/*
		 * If user specified the optimizer override for a fixed join order, then
		 * desiredJoinOrderFound could be true when we get here. We have to
		 * reset it to false in prep for the next round of optimization.
		 * Otherwise we'd end up quitting the optimization before ever finding a
		 * plan for this round, and that could, among other things, lead to a
		 * never-ending optimization phase in certain situations. DERBY-1866.
		 */
	...
	public boolean getNextPermutation() throws StandardException {
		...
		/*
		 * Make sure that all Optimizables init their access paths. (They wait
		 * until optimization since the access path references the optimizer.)
		 */
		optimizableList.initAccessPaths(this);

		/*
		 * * Experiments show that optimization time only starts to* become a
		 * problem with seven tables, so only check for* too much time if there
		 * are more than seven tables.* Also, don't check for too much time if
		 * user has specified* no timeout.
		 */
		...
		if (bestCost.isUninitialized()
			...
			/*
			 * We can get here if this OptimizerImpl is for a subquery that
			 * timed out for a previous permutation of the outer query, but then
			 * the outer query itself did _not_ timeout. In that case we'll end
			 * up back here for another round of optimization, but our
			 * timeExceeded flag will be true. We don't want to reset all of the
			 * timeout state here because that could lead to redundant work (see
			 * comments in prepForNextRound()), but we also don't want to return
			 * without having a plan, because then we'd return an unfairly high
			 * "bestCost" value--i.e. Double.MAX_VALUE. Note that we can't just
			 * revert back to whatever bestCost we had prior to this because
			 * that cost is for some previous permutation of the outer
			 * query--not the current permutation-- and thus would be incorrect.
			 * So instead we have to delay the timeout until we find a complete
			 * (and valid) join order, so that we can return a valid cost
			 * estimate. Once we have a valid cost we'll then go through the
			 * timeout logic and stop optimizing.
			 * 
			 * All of that said, instead of just trying the first possible join
			 * order, we jump to the join order that gave us the best cost in
			 * previous rounds. We know that such a join order exists because
			 * that's how our timeout value was set to begin with--so if there
			 * was no best join order, we never would have timed out and thus we
			 * wouldn't be here.
			 * 
			 * We can also get here if we've already optimized the list of
			 * optimizables once (in a previous round of optimization) and now
			 * we're back to do it again. If that's true AND we did *not*
			 * receive any predicates pushed from above AND the bestJoinOrder
			 * from the previous round did *not* depend on predicates pushed
			 * from above, then we'll jump to the previous join order and start
			 * there. NOTE: if after jumping to the previous join order and
			 * calculating the cost we haven't timed out, we will continue
			 * looking at other join orders (as usual) until we exhaust them all
			 * or we time out.
			 */
			if (permuteState != JUMPING) {
				...
				if (firstLookOrder == null)
					firstLookOrder = new int[numOptimizables];
				System.arraycopy(bestJoinOrder, 0, firstLookOrder, 0,
						numOptimizables);
				permuteState = JUMPING;

				/*
				 * If we already assigned at least one position in the join
				 * order when this happened (i.e. if joinPosition is greater
				 * than *or equal* to zero; DERBY-1777), then reset the join
				 * order before jumping. The call to rewindJoinOrder() here will
				 * put joinPosition back to 0. But that said, we'll then end up
				 * incrementing joinPosition before we start looking for the
				 * next join order (see below), which means we need to set it to
				 * -1 here so that it gets incremented to "0" and then
				 * processing can continue as normal from there. Note: we don't
				 * need to set reloadBestPlan to true here because we only get
				 * here if we have *not* found a best plan yet.
				 */
				if (joinPosition >= 0) {
					rewindJoinOrder();
					joinPosition = -1;
		...
		/*
		 * * Pick the next table in the join order, if there is an unused
		 * position* in the join order, and the current plan is less expensive
		 * than* the best plan so far, and the amount of time spent optimizing
		 * is* still less than the cost of the best plan so far, and a best*
		 * cost has been found in the current join position. Otherwise,* just
		 * pick the next table in the current position.
		 */
		boolean joinPosAdvanced = false;

		/*
		 * Determine if the current plan is still less expensive than the best
		 * plan so far. If bestCost is uninitialized then we want to return
		 * false here; if we didn't, then in the (rare) case where the current
		 * cost is greater than Double.MAX_VALUE (esp. if it's
		 * Double.POSITIVE_INFINITY, which can occur for very deeply nested
		 * queries with long FromLists) we would give up on the current plan
		 * even though we didn't have a best plan yet, which would be wrong.
		 * Also note: if we have a required row ordering then we might end up
		 * using the sort avoidance plan--but we don't know at this point which
		 * plan (sort avoidance or "normal") we're going to use, so we error on
		 * the side of caution and only short- circuit if both currentCost and
		 * currentSortAvoidanceCost (if the latter is applicable) are greater
		 * than bestCost.
		 */
		boolean alreadyCostsMore = !bestCost.isUninitialized()
				&& (currentCost.compare(bestCost) > 0)
				&& ((requiredRowOrdering == null) || (currentSortAvoidanceCost
						.compare(bestCost) > 0));

		if ((joinPosition < (numOptimizables - 1)) && !alreadyCostsMore
				&& (!timeExceeded)) {
			/*
			 * * Are we either starting at the first join position (in which*
			 * case joinPosition will be -1), or has a best cost been found* in
			 * the current join position? The latter case might not be* true if
			 * there is no feasible join order.
			 */
			if ((joinPosition < 0)
					|| optimizableList
				...
				/*
				 * * When adding a table to the join order, the best row* row
				 * ordering for the outer tables becomes the starting* point for
				 * the row ordering of the current table.
				 */
		...
		while (joinPosition >= 0) {
			...
			if (proposedJoinOrder[joinPosition] >= 0) {
				/*
				 * We are either going to try another table at the current join
				 * order position, or we have exhausted all the tables at the
				 * current join order position. In either case, we need to pull
				 * the table at the current join order position and remove it
				 * from the join order. Do this BEFORE we search for the next
				 * optimizable so that assignedTableMap, which is updated to
				 * reflect the PULL, has the correct information for enforcing
				 * join order depdendencies. DERBY-3288.
				 */
			...
			if (desiredJoinOrderFound || timeExceeded) {
				 ...
				 * * us to back out of the current join order.** Also, don't
				 * look at any more join orders if we have taken* too much time
			...
			} else if (permuteState == JUMPING) // still jumping
			{
				 ...
				 * with the lowest estimated costs first (insofar as it* is
				 * legal to do so). The "firstLookOrder" array holds the* ideal
				 * join order for position <joinPosition> up thru* position
				 * <numOptimizables-1>. So here, we look at the* ideal
				 * optimizable to place at <joinPosition> and see if* it's
				 * legal; if it is, then we're done. Otherwise, we* swap it with
				 * <numOptimizables-1> and see if that gives us* a legal join
				 * order w.r.t <joinPosition>. If not, then we* swap it with
				 * <numOptimizables-2> and check, and if that* fails, then we
				 * swap it with <numOptimizables-3>, and so* on. For example,
				 * assume we have 6 optimizables whose* order from least
				 * expensive to most expensive is 2, 1, 4,* 5, 3, 0. Assume also
				 * that we've already verified the* legality of the first two
				 * positions--i.e. that joinPosition* is now "2". That means
				 * that "firstLookOrder" currently* contains the following:** [
				 * pos ] 0 1 2 3 4 5* [ opt ] 2 1 4 5 3 0** Then at this point,
				 * we do the following:** -- Check to see if the ideal
				 * optimizable "4" is valid* at its current position (2)* -- If
				 * opt "4" is valid, then we're done; else we* swap it with the
				 * value at position _5_:** [ pos ] 0 1 2 3 4 5* [ opt ] 2 1 0 5
				 * 3 4** -- Check to see if optimizable "0" is valid at its* new
				 * position (2).* -- If opt "0" is valid, then we're done; else
				 * we* put "0" back in its original position and swap* the ideal
				 * optimizer ("4") with the value at* position _4_:** [ pos ] 0
				 * 1 2 3 4 5* [ opt ] 2 1 3 5 4 0** -- Check to see if
				 * optimizable "3" is valid at its* new position (2).* -- If opt
				 * "3" is valid, then we're done; else we* put "3" back in its
				 * original position and swap* the ideal optimizer ("4") with
				 * the value at* position _3_:** [ pos ] 0 1 2 3 4 5* [ opt ] 2
				 * 1 5 4 3 0** -- Check to see if optimizable "5" is valid at
				 * its* new position (2).* -- If opt "5" is valid, then we're
				 * done; else we've* tried all the available optimizables and
				 * none* of them are legal at position 2. In this case,* we give
				 * up on "JUMPING" and fall back to normal* join-order
				 * processing.
				 */
				...
				Optimizable nextOpt;
				for (nextOpt = optimizableList.getOptimizable(nextOptimizable); !(nextOpt
						.legalJoinOrder(assignedTableMap)); nextOpt = optimizableList
						.getOptimizable(nextOptimizable)) {
				...
				for (; nextOptimizable < numOptimizables; nextOptimizable++) {
					...
					for (int posn = 0; posn < joinPosition; posn++) {
						/*
						 * * Is this optimizable already somewhere* in the join
						 * order?
						 */
					...
					/*
					 * No need to check the dependencies if the optimizable is
					 * already in the join order--because we should have checked
					 * its dependencies before putting it there.
					 */
					if (found) {
						if (SanityManager.DEBUG) {
					...
					if ((nextOptimizable < numOptimizables)
							&& !joinOrderMeetsDependencies(nextOptimizable)) {
						if (tracingIsOn()) {
							tracer().traceSkippingJoinOrder(nextOptimizable,
									joinPosition,
									ArrayUtil.copy(proposedJoinOrder),
									(JBitSet) assignedTableMap.clone());
						}

						/*
						 * * If this is a user specified join order then it is
						 * illegal.
						 */
						if (!optimizableList.optimizeJoinOrder()) {
							...
							throw StandardException
									.newException(SQLState.LANG_ILLEGAL_FORCED_JOIN_ORDER);
						}
			...
			if (nextOptimizable >= numOptimizables) {
				...
				if (permuteState == READY_TO_JUMP && joinPosition > 0
						&& joinPosition == numOptimizables - 1) {
					permuteState = JUMPING;

					/*
					 * A simple heuristics is that the row count we got
					 * indicates a potentially good join order. We'd like row
					 * count to get big as late as possible, so that less load
					 * is carried over.
					 */
					...
					if (permuteState == JUMPING) {
						boolean doIt = false;
						int temp;
						for (int i = 0; i < numOptimizables; i++) // simple
																	// selection
																	// sort
						{
							int k = i;
						...
						if (doIt) {
							...
							permuteState = NO_JUMP; // never
					}
				}

				/*
				 * * We have exhausted all the optimizables at this level.* Go
				 * back up one level.
				 */
				...
				if (joinPosition < 0 && permuteState == WALK_HIGH) // reached
																	// peak
				{
			...
			/*
			 * Re-init (clear out) the cost for the best access path when
			 * placing a table.
			 */
			optimizableList.getOptimizable(nextOptimizable).getBestAccessPath()
					.setCostEstimate((CostEstimate) null);

			if (tracingIsOn()) {
				tracer().traceJoinOrderConsideration(joinPosition,
						ArrayUtil.copy(proposedJoinOrder),
						(JBitSet) assignedTableMap.clone());
			}
			 ...
			 * Optimizable in the current join order. Assumption is that this OR
			 * can always be undone using an XOR, which will only be true if
			 * none of the Optimizables have overlapping table maps. The XOR
			 * itself occurs as part of optimizable "PULL" processing.
			 */
			if (SanityManager.DEBUG) {
				JBitSet optMap = (JBitSet) nextOpt.getReferencedTableMap()
						.clone();

				optMap.and(assignedTableMap);
				if (optMap.getFirstSetBit() != -1) {
					SanityManager
							.THROWASSERT("Found multiple optimizables that share one or "
			...
			pushPredicates(optimizableList.getOptimizable(nextOptimizable),
					assignedTableMap);
	...
	private void pullOptimizableFromJoinOrder() throws StandardException {
		...
		/*
		 * * Subtract the cost estimate of the optimizable being* removed from
		 * the total cost estimate.** The total cost is the sum of all the
		 * costs, but the total* number of rows is the number of rows returned
		...
		/*
		 * * If there is no feasible join order, the cost estimate* in the best
		 * access path may never have been set.* In this case, do not subtract
		 * anything from the* current cost, since nothing was added to the
		 * current* cost.
		 */
		double newCost = currentCost.getEstimatedCost();
		CostEstimate pullCostEstimate = pullMe.getBestAccessPath()
				.getCostEstimate();
		if (pullCostEstimate != null) {
			...
			/*
			 * * It's possible for newCost to go negative here due to* loss of
			 * precision--but that should ONLY happen if the* optimizable we
			 * just pulled was at position 0. If we* have a newCost that is <= 0
			 * at any other time, then* it's the result of a different kind of
			 * precision loss--* namely, the estimated cost of pullMe was so
			 * large that* we lost the precision of the accumulated cost as it*
			 * existed prior to pullMe. Then when we subtracted* pullMe's cost
			 * out, we ended up setting newCost to zero.* That's an unfortunate
			 * side effect of optimizer cost* estimates that grow too large. If
			 * that's what happened* here,try to make some sense of things by
			 * adding up costs* as they existed prior to pullMe...
			 */
			if (newCost <= 0.0) {
				if (joinPosition == 0)
					newCost = 0.0;
		...
		/*
		 * If we are choosing a new outer table, then we rest the starting cost
		 * to the outermostCost. (Thus avoiding any problems with floating point
		...
		/*
		 * * Subtract from the sort avoidance cost if there is a* required row
		 * ordering.** NOTE: It is not necessary here to check whether the* best
		 * cost was ever set for the sort avoidance path,* because it
		 * considerSortAvoidancePath() would not be* set if there cost were not
		 * set.
		 */
		if (requiredRowOrdering != null) {
			if (pullMe.considerSortAvoidancePath()) {
				 ...
				 * optimizable being removed from the total sort* avoidance cost
				 * estimate.** The total cost is the sum of all the costs, but
				 * the* total number of rows is the number of rows returned* by
				 * the innermost optimizable.
				 */
				if (joinPosition == 0) {
					prevRowCount = outermostCostEstimate.rowCount();
					prevSingleScanRowCount = outermostCostEstimate
							.singleScanRowCount();

					/*
					 * If we are choosing a new outer table, then we rest the
					 * starting cost to the outermostCost. (Thus avoiding any
					 * problems with floating point accuracy and going
					 * negative.)
					 */
					prevEstimatedCost = outermostCostEstimate
							.getEstimatedCost();
				} else {
					CostEstimate localCE = optimizableList
							.getOptimizable(prevPosition)
							.getBestSortAvoidancePath().getCostEstimate();
					prevRowCount = localCE.rowCount();
					prevSingleScanRowCount = localCE.singleScanRowCount();
					prevEstimatedCost = currentSortAvoidanceCost
							.getEstimatedCost()
				...
				currentSortAvoidanceCost.setCost(prevEstimatedCost,
						prevRowCount, prevSingleScanRowCount);

				/*
				 * * Remove the table from the best row ordering.* It should not
				 * be necessary to remove it from* the current row ordering,
				 * because it is* maintained as we step through the access paths
				 * * for the current Optimizable.
				 */
				bestRowOrdering.removeOptimizable(pullMe.getTableNumber());

				/*
				 * * When removing a table from the join order,* the best row
				 * ordering for the remaining outer tables* becomes the starting
		...
		/*
		 * * Pull the predicates at from the optimizable and put* them back in
		 * the predicate list.** NOTE: This is a little inefficient because it
		 * pulls the* single-table predicates, which are guaranteed to always*
		 * be pushed to the same optimizable. We could make this* leave the
		 * single-table predicates where they are.
		 */
		pullMe.pullOptPredicates(predicateList);

		/*
		 * * When we pull an Optimizable we need to go through and* load
		 * whatever best path we found for that Optimizable* with respect to
		 * this OptimizerImpl. The reason is that* we could be pulling the
		 * Optimizable for the last time* (before returning false), in which
		 * case we want it (the* Optimizable) to be holding the best access path
		 * that it* had at the time we found bestJoinOrder. This ensures* that
		 * the access path which is generated and executed for* the Optimizable
		 * matches the the access path decisions* made by this OptimizerImpl for
		 * the best join order.** NOTE: We we only reload the best plan if it's
		 * necessary* to do so--i.e. if the best plans aren't already loaded.*
		 * The plans will already be loaded if the last complete* join order we
		 * had was the best one so far, because that* means we called
		 * "rememberAsBest" on every Optimizable* in the list and, as part of
		 * that call, we will run through* and set trulyTheBestAccessPath for
		 * the entire subtree.* So if we haven't tried any other plans since
		 * then,* we know that every Optimizable (and its subtree) already* has
		 * the correct best plan loaded in its trulyTheBest* path field. It's
		 * good to skip the load in this case* because 'reloading best plans'
		 * involves walking the* entire subtree of _every_ Optimizable in the
		 * list, which* can be expensive if there are deeply nested subqueries.
		 */
		if (reloadBestPlan)
			pullMe.updateBestPlanMap(FromTable.LOAD_PLAN, this);
		...
		/*
		 * If we didn't advance the join position then the optimizable which
		 * currently sits at proposedJoinOrder[joinPosition]--call it
		 * PULL_ME--is *not* going to remain there. Instead, we're going to pull
		 * that optimizable from its position and attempt to put another one in
		 * its place. That said, when we try to figure out which of the other
		 * optimizables to place at joinPosition, we'll first do some
		 * "dependency checking", the result of which relies on the contents of
		 * assignedTableMap. Since assignedTableMap currently holds info about
		 * PULL_ME and since PULL_ME is *not* going to remain in the join order,
		 * we need to remove the info for PULL_ME from assignedTableMap.
		 * Otherwise an Optimizable which depends on PULL_ME could incorrectly
		 * be placed in the join order *before* PULL_ME, which would violate the
	...
	public boolean getNextDecoratedPermutation() throws StandardException {
		boolean retval;
		Optimizable curOpt = optimizableList
				.getOptimizable(proposedJoinOrder[joinPosition]);
		...
		while (true) {
			...
			retval = curOpt.nextAccessPath(this,
					(OptimizablePredicateList) null, currentRowOrdering);
			...
			// at this point, figure out if the plan so far is a prefix of the
			// desired plan
			OptimizerPlan candidate = OptimizerPlan.makeRowSource(
					getTupleDescriptor(curOpt), dDictionary);
			if (candidate == null) {
				retval = false;
				break;
			}
			if (currentPlan != null) {
				candidate = new OptimizerPlan.Join(curOpt
						.getCurrentAccessPath().getJoinStrategy(), currentPlan,
						candidate);
			}
			...
			// well, that decoration didn't match up with the user-specified
			// plan.
			// try again
		}
		...
		if ((curOpt.getBestAccessPath().getCostEstimate() != null)
			...
			if (curOpt.getBestAccessPath().getCostEstimate()
					.compare(curOpt.getCurrentAccessPath().getCostEstimate()) != 0) {
				curOpt.updateBestPlanMap(FromTable.LOAD_PLAN, curOpt);
			} else if (curOpt.getBestAccessPath().getCostEstimate().rowCount() < curOpt
					.getCurrentAccessPath().getCostEstimate().rowCount()) {
		...
		/*
		 * If we needed to revert plans for curOpt, we just did it above. So we
		 * no longer need to keep the previous best plan--and in fact, keeping
		 * it can lead to extreme memory usage for very large queries. So delete
		 * the stored plan for curOpt. DERBY-1315.
		 */
		curOpt.updateBestPlanMap(FromTable.REMOVE_PLAN, curOpt);

		/*
		 * * When all the access paths have been looked at, we know what the*
		 * cheapest one is, so remember it. Only do this if a cost estimate* has
		 * been set for the best access path - one will not have been* set if no
		 * feasible plan has been found.
		 */
		CostEstimate ce = curOpt.getBestAccessPath().getCostEstimate();
		if ((!retval) && (ce != null)) {
			/*
			 * * Add the cost of the current optimizable to the total cost.* The
			 * total cost is the sum of all the costs, but the total* number of
			 * rows is the number of rows returned by the innermost*
			 * optimizable.
			 */
			currentCost.setCost(
					currentCost.getEstimatedCost() + ce.getEstimatedCost(),
					ce.rowCount(), ce.singleScanRowCount());
			...
			if (curOpt.considerSortAvoidancePath()
				...
				currentSortAvoidanceCost.setCost(
						currentSortAvoidanceCost.getEstimatedCost()
								+ ce.getEstimatedCost(), ce.rowCount(),
			...
			if (tracingIsOn()) {
				...
				if (curOpt.considerSortAvoidancePath()) {
					tracer().traceCostWithSortAvoidance(
			...
			if (joinPosition == (numOptimizables - 1)) {
				...
				if (requiredRowOrdering != null) {
					 ...
					 * sort-needed or not, as done in rememberBestCost. If the
					 * bestCost so far is sort-needed, and assume currentCost is
					 * also sort-needed, we want this comparison to be as
					 * accurate as possible. Different plans may produce
					 * different estimated row count (eg., heap scan vs. index
					 * scan during a join), sometimes the difference could be
					 * very big. However the actual row count should be only one
					 * value. So when comparing these two plans, we want them to
					 * have the same sort cost. We want to take the smaller row
					 * count, because a better estimation (eg. through index)
					 * would yield a smaller number. We adjust the bestCost here
					 * if it had a bigger rowCount estimate. The performance
					 * improvement of doing this sometimes is quite dramatic,
					 * eg. from 17 sec to 0.5 sec, see beetle 4353.
					 */
					else if (requiredRowOrdering.getSortNeeded()) {
						if (bestCost.rowCount() > currentCost.rowCount()) {
							// adjust bestCost
							requiredRowOrdering.estimateCost(
									bestCost.rowCount(), bestRowOrdering,
									sortCost);
							double oldSortCost = sortCost.getEstimatedCost();
							requiredRowOrdering.estimateCost(
									currentCost.rowCount(), bestRowOrdering,
									sortCost);
							gotSortCost = true;
							bestCost.setCost(
									bestCost.getEstimatedCost() - oldSortCost
					...
					/*
					 * This does not figure out if sorting is necessary, just an
					 * asumption that sort is needed; if the assumption is
					...
					currentCost.setCost(currentCost.getEstimatedCost()
							+ sortCost.getEstimatedCost(), sortCost.rowCount(),
							currentCost.singleScanRowCount());
				...
				/*
				 * * Is the cost of this join order lower than the best one
				 * we've* found so far?** NOTE: If the user has specified a join
				 * order, it will be the* only join order the optimizer
				 * considers, so it is OK to use* costing to decide that it is
				 * the "best" join order.** For very deeply nested queries, it's
				 * possible that the optimizer* will return an estimated cost of
				 * Double.INFINITY, which is* greater than our uninitialized
				 * cost of Double.MAX_VALUE and* thus the "compare" check below
				 * will return false. So we have* to check to see if bestCost is
				 * uninitialized and, if so, we* save currentCost regardless of
				 * what value it is--because we* haven't found anything better
				 * yet.** That said, it's also possible for bestCost to be
				 * infinity* AND for current cost to be infinity, as well. In
				 * that case* we can't really tell much by comparing the two, so
				 * for lack* of better alternative we look at the row counts.
				 * See* CostEstimateImpl.compare() for more.
				 */
				...
				if (requiredRowOrdering != null) {
					...
					currentCost.setCost(newCost, originalRowCount,
							currentCost.singleScanRowCount());
				}
				...
				if (requiredRowOrdering != null
					...
					if (requiredRowOrdering.sortRequired(bestRowOrdering,
							optimizableList, proposedJoinOrder) == RequiredRowOrdering.NOTHING_REQUIRED) {
						if (tracingIsOn()) {
							tracer().traceCurrentPlanAvoidsSort(bestCost,
									currentSortAvoidanceCost);
						}
	...
	/**
	 * Is the cost of this join order lower than the best one we've found so
	 * far? If so, remember it.
	 *
	 * NOTE: If the user has specified a join order, it will be the only join
	 * order the optimizer considers, so it is OK to use costing to decide that
	 * it is the "best" join order.
	 *
	 * @exception StandardException
	 *                Thrown on error
	 */
	private void rememberBestCost(CostEstimate currentCost, int planType)
		...
		if (tracingIsOn()) {
			tracer().traceCheapestPlanSoFar(planType, currentCost);
		}
		...
		/*
		 * * Remember the current join order and access path* selections as
		 * best.* NOTE: We want the optimizer trace to print out in* join order
		 * instead of in table number order, so* we use 2 loops.
		 */
		bestJoinOrderUsedPredsFromAbove = usingPredsPushedFromAbove;

		System.arraycopy(proposedJoinOrder, 0, bestJoinOrder, 0,
				numOptimizables);

		for (int i = 0; i < numOptimizables; i++) {
			optimizableList.getOptimizable(bestJoinOrder[i]).rememberAsBest(
					planType, this);
		}
		...
		if (tracingIsOn()) {
			if (requiredRowOrdering != null) {
				tracer().traceSortNeededForOrdering(planType,
						requiredRowOrdering);
			}
			tracer().traceRememberingBestJoinOrder(joinPosition,
					ArrayUtil.copy(bestJoinOrder), planType, currentCost,
					(JBitSet) assignedTableMap.clone());
		}
	}
	...
	public void costPermutation() throws StandardException {
		...
		if (joinPosition == 0) {
			 ...
			 * outer cost from the best access path for the outer table* is OK
			 * to use even when costing the sort avoidance path for* the inner
			 * table. This is probably OK, since all we use* from the outer cost
			 * is the row count.
			 */
			outerCost = optimizableList
					.getOptimizable(proposedJoinOrder[joinPosition - 1])
					.getBestAccessPath().getCostEstimate();
		}

		/*
		 * At this point outerCost should be non-null (DERBY-1777). Do the
		 * assertion here so that we catch it right away; otherwise we'd end up
		 * with an NPE somewhere further down the tree and it'd be harder to
		...
		Optimizable optimizable = optimizableList
				.getOptimizable(proposedJoinOrder[joinPosition]);
		...
		optimizable.optimizeIt(this, predicateList, outerCost,
				currentRowOrdering);
	}
	 ...
	 *                Thrown on error
	 */
	public void costOptimizable(Optimizable optimizable, TableDescriptor td,
			ConglomerateDescriptor cd, OptimizablePredicateList predList,
		...
		/*
		 * * Classify the predicates according to the given conglomerate.* The
		 * predicates are classified as start keys, stop keys,* qualifiers, and
		 * none-of-the-above. They are also ordered* to match the ordering of
		 * columns in keyed conglomerates (no* ordering is done for heaps).
		 */
		// if (predList != null)
	...
	/**
	 * This method decides whether the given conglomerate descriptor is cheapest
	 * based on rules, rather than based on cost estimates. The rules are:
	 *
	 * Covering matching indexes are preferred above all Non-covering matching
	 * indexes are next in order of preference Covering non-matching indexes are
	 * next in order of preference Heap scans are next in order of preference
	 * Non-covering, non-matching indexes are last in order of preference.
	 *
	 * In the current language architecture, there will always be a heap, so a
	 * non-covering, non-matching index scan will never be chosen. However, the
	 * optimizer may see a non-covering, non-matching index first, in which case
	 * it will choose it temporarily as the best conglomerate seen so far.
	 *
	 * NOTE: This method sets the cost in the optimizable, even though it
	 * doesn't use the cost to determine which access path to choose. There are
	 * two reasons for this: the cost might be needed to determine join order,
	 * and the cost information is copied to the query plan.
	 */
	private void ruleBasedCostOptimizable(Optimizable optimizable,
			TableDescriptor td, ConglomerateDescriptor cd,
			OptimizablePredicateList predList, CostEstimate outerCost)
			throws StandardException {
		/* CHOOSE BEST CONGLOMERATE HERE */
		AccessPath bestAp = optimizable.getBestAccessPath();

		/*
		 * * If the current conglomerate better than the best so far?* The
		 * pecking order is:* o covering index useful for predicates* (if there
		 * are predicates)* o index useful for predicates (if there are
		 * predicates)* o covering index* o table scan
		 */

		/*
		 * * If there is more than one conglomerate descriptor* choose any index
		...
		if (predList != null && predList.useful(optimizable, cd)) {
			...
			if ((!bestAp.getCoveringIndexScan())
					|| bestAp.getNonMatchingIndexScan() || newCoveringIndex) {
				bestAp.setCostEstimate(estimateTotalCost(predList, cd,
						outerCost, optimizable));
				bestAp.setConglomerateDescriptor(cd);
				bestAp.setNonMatchingIndexScan(false);
				bestAp.setCoveringIndexScan(newCoveringIndex);

				bestAp.setLockMode(optimizable.getCurrentAccessPath()
						.getLockMode());
		...
		/*
		 * Remember the "last" covering index. NOTE - Since we don't have
		 * costing, we just go for the last one since that's as good as any
		 */
		if (optimizable.isCoveringIndex(cd)) {
			bestAp.setCostEstimate(estimateTotalCost(predList, cd, outerCost,
					optimizable));
			bestAp.setConglomerateDescriptor(cd);
		 ...
		 */
		if ((!bestAp.getCoveringIndexScan())
				&& bestAp.getNonMatchingIndexScan() && (!cd.isIndex())) {
			bestAp.setCostEstimate(estimateTotalCost(predList, cd, outerCost,
					optimizable));
			...
			/*
			 * * No need to set non-matching index scan and covering* index
			 * scan, as these are already correct.
			 */
			return;
		}

		/*
		 * * If all else fails, and no conglomerate has been picked yet,* pick
		 * this one.
		 */
		ConglomerateDescriptor bestConglomerateDescriptor = bestAp
				.getConglomerateDescriptor();
	...
	/**
	 * This method decides whether the given conglomerate descriptor is cheapest
	 * based on cost, rather than based on rules. It compares the cost of using
	 * the given ConglomerateDescriptor with the cost of using the best
	 * ConglomerateDescriptor so far.
	 */
	private void costBasedCostOptimizable(Optimizable optimizable,
			TableDescriptor td, ConglomerateDescriptor cd,
			OptimizablePredicateList predList, CostEstimate outerCost)
			throws StandardException {
		CostEstimate estimatedCost = estimateTotalCost(predList, cd, outerCost,
				optimizable);
		...
		/*
		 * * Skip this access path if it takes too much memory.** NOTE: The
		 * default assumption here is that the number of rows in* a single scan
		 * is the total number of rows divided by the number* of outer rows. The
		 * optimizable may over-ride this assumption.
		 */
		// RESOLVE: The following call to memoryUsageOK does not behave
		// correctly if outerCost.rowCount() is POSITIVE_INFINITY; see
		// DERBY-1259.
		if (!optimizable.memoryUsageOK(
		...
		if ((bestCostEstimate == null) || bestCostEstimate.isUninitialized()
			...
			/*
			 * * It's a non-matching index scan either if there is no* predicate
			 * list, or nothing in the predicate list is useful* for limiting
			 * the scan.
			 */
			ap.setNonMatchingIndexScan((predList == null)
		...
		/*
		 * * Keep track of the best sort-avoidance path if there is a* required
		 * row ordering.
		 */
		if (requiredRowOrdering != null) {
			/*
			 * * The current optimizable can avoid a sort only if the* outer one
			 * does, also (if there is an outer one).
			 */
			if (joinPosition == 0
				...
				if (requiredRowOrdering.sortRequired(currentRowOrdering,
						assignedTableMap, optimizableList, proposedJoinOrder) == RequiredRowOrdering.NOTHING_REQUIRED) {
					...
					if ((bestCostEstimate == null)
						...
						/*
						 * * It's a non-matching index scan either if there is
						 * no* predicate list, or nothing in the predicate list
						 * is* useful for limiting the scan.
						 */
						ap.setNonMatchingIndexScan((predList == null)
								|| (!(predList.useful(optimizable, cd))));
						ap.setLockMode(optimizable.getCurrentAccessPath()
								.getLockMode());
						optimizable.rememberJoinStrategyAsBest(ap);
	...
	public void considerCost(Optimizable optimizable,
			OptimizablePredicateList predList, CostEstimate estimatedCost,
			CostEstimate outerCost) throws StandardException {
		/*
		 * * Don't consider non-feasible join strategies.
		 */
		...
		/*
		 * Pick the cheapest cost for this particular optimizable. NOTE:
		 * Originally, the code only chose the new access path if it was cheaper
		 * than the old access path. However, I (Jerry) found that the new and
		 * old costs were the same for a derived table and the old access path
		 * did not have a join strategy associated with it in that case. So, we
		 * now choose the new access path if it is the same cost or cheaper than
	 ...
	 *
	 *      Sum up the cost of all of the trulyTheBestAccessPaths for the
	 *      Optimizables in our list. Assumption is that we only get here after
	 *      optimization has completed--i.e. while modifying access paths.
	 */
	public CostEstimate getFinalCost() {
		...
		for (int i = 0; i < bestJoinOrder.length; i++) {
			...
			finalCostEstimate.setCost(
					finalCostEstimate.getEstimatedCost()
							+ ce.getEstimatedCost(), ce.rowCount(),
					ce.singleScanRowCount());
		}
	...
	public void setOuterRows(double outerRows) {
		outermostCostEstimate.setCost(outermostCostEstimate.getEstimatedCost(),
				outerRows, outermostCostEstimate.singleScanRowCount());
	}
	...
	private CostEstimate estimateTotalCost(OptimizablePredicateList predList,
		...
		CostEstimate resultCost = optimizable.estimateCost(predList, cd,
				outerCost, this, currentRowOrdering);
	...
	CostEstimateImpl getNewCostEstimate(double theCost, double theRowCount,
