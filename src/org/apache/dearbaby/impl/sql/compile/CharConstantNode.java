...
public final class CharConstantNode extends ConstantNode {
	...
	CharConstantNode(String value, ContextManager cm) throws StandardException {
		super(TypeId.CHAR_ID, value == null, // nullable?
				(value != null) ? value.length() : 0, cm);

		// setValue(getDataValueFactory().getCharDataValue(value));
		DataValueFactory d = new DataValueFactoryImpl();

		setValue(d.getCharDataValue(value));
		kind = K_CHAR;
	}
	...
	CharConstantNode(String newValue, int newLength, ContextManager cm)
		...
		if (newValue.length() > newLength) {
			throw StandardException.newException(
					SQLState.LANG_STRING_TRUNCATION, "CHAR", newValue,
	...
	/**
	 * Return an Object representing the bind time value of this expression
	 * tree. If the expression tree does not evaluate to a constant at bind time
	 * then we return null. This is useful for bind time resolution of VTIs.
	 * RESOLVE: What do we do for primitives?
	 *
	 * @return An Object representing the bind time value of this expression
	 *         tree. (null if not a bind time constant.)
	 *
	 * @exception StandardException
	 *                Thrown on error
	 */
	...
	ValueNode bindExpression(FromList fromList, SubqueryList subqueryList,
		...
		value = ((StringDataValue) value)
				.getValue(getLanguageConnectionContext().getDataValueFactory()
	...
	/**
	 * This generates the proper constant. It is implemented by every specific
	 * constant node (e.g. IntConstantNode).
	 *
	 * @param acb
