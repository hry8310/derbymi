...
class CastNode extends ValueNode {
	...
	/**
	 * This variable gets set by the parser to indicate that this CAST node has
	 * been generated by the parser. This means that we should use the collation
	 * info of the current compilation schema for this node's collation setting.
	 * If this variable does not get set to true, then it means that this CAST
	 * node has been an internally generated node and we should not touch the
	 * collation info set for this CAST node because it has been already set
	 * correctly by the class that generated this CAST node. Collation info is
	 * part of the DataTypeDescriptor that's defined on the ValueNode (the super
	 * class of this CastNode class)
	 */
	private boolean externallyGeneratedCastNode = false;

	/*
	 * * Static array of valid casts. Dimentions* produce a single boolean which
	 * indicates* whether the case is possible or not.
	 */

	/**
	 * Method calls: Argument type has the same semantics as assignment: Section
	 * 9.2 (Store assignment). There, General Rule 2.b.v.2 says that the
	 * database should raise an exception if truncation occurs when stuffing a
	 * string value into a VARCHAR, so make sure CAST doesn't issue warning
	 * only.
	 */
	private boolean assignmentSemantics = false;

	/**
	 * The name of the target type if it's a UDT. It is partly redundant, as the
	 * name can also be retrieved from the type descriptor. Additionally, it
	 * contains information about the location of the UDT name in the query
	 * text, which is useful if the query text needs to be rewritten. (Useful
	 * for example when rewriting a CHECK constraint definition to have fully
	 * qualified names before storing it in the dictionary.) This field is only
	 * set for <b>explicit</b> casts to a UDT.
	 */
	 ...
	 * @param castTarget
	 *            DataTypeServices (target type of cast)
	 * @param cm
	 *            The context manager
	 *
	...
	CastNode(ValueNode castOperand, DataTypeDescriptor castTarget,
	 ...
	 *            The operand of the node
	 * @param charType
	 *            CHAR or VARCHAR JDBC type as target
	 * @param charLength
	 *            target type length
	 * @param cm
	 *            The context manager
	...
	CastNode(ValueNode castOperand, int charType, int charLength,
			ContextManager cm) throws StandardException {
		super(cm);
		this.castOperand = castOperand;
		...
		if (charLen < 0) // unknown, figure out later
			return;
		setType(DataTypeDescriptor.getBuiltInDataTypeDescriptor(targetCharType,
				charLen));
	}
	...
	ValueNode bindExpression(FromList fromList, SubqueryList subqueryList,
		...
		castOperand = castOperand.bindExpression(fromList, subqueryList,
				aggregates);
		...
		if (getTypeServices() == null) // CHAR or VARCHAR function without
										// specifying target length
		{
			 ...
			 * incorrectly while Char & Varchar functions were used. Thus adding
			 * the check for Char & Varchar and calculating the length based on
			 * the operand type.
			 */
			...
			setType(DataTypeDescriptor.getBuiltInDataTypeDescriptor(
					targetCharType, length));

		}

		/*
		 * * If castOperand is an untyped null,* then we must set the type.
		 */
		...
		/*
		 * We can't chop out cast above an untyped null because the store can't
		 * handle it.
		 */
		if ((castOperand instanceof ConstantNode)
			 ...
			 * bind time and return a constant of the correct type. NOTE: This
			 * could return an exception, but we're prepared to deal with that.
			 * (NumberFormatException, etc.) We only worry about the easy (and
			 * useful) converions at bind time. Here's what we support: source
			 * destination ------ ----------- boolean boolean boolean char char
			 * boolean char date/time/ts char non-decimal numeric date/time/ts
			 * char numeric char numeric non-decimal numeric
			 */
			...
			case Types.BOOLEAN:
				// (BIT is boolean)
				if (destJDBCTypeId == Types.BIT
						|| destJDBCTypeId == Types.BOOLEAN) {
					retNode = castOperand;
					...
					retNode = new CharConstantNode(booleanString,
			...
			case Types.TIMESTAMP:
				if (destJDBCTypeId == Types.CHAR) {
					String castValue = ((UserTypeConstantNode) castOperand)
							.getObjectValue().toString();
					retNode = new CharConstantNode(castValue, getTypeServices()
							.getMaximumWidth(), getContextManager());
				}
				break;
			...
			case Types.REAL:
				retNode = getCastFromNumericType(
						((ConstantNode) castOperand).getValue(), destJDBCTypeId);
				break;
	...
	/**
	 * Bind this node but not its child. Caller has already bound the child.
	 * This is useful for when we generate a CastNode during binding after
	 * having already bound the child.
	 *
	 * @exception StandardException
	 *                Thrown on error
	 */
	void bindCastNodeOnly() throws StandardException {
		...
		/*
		 * * If it is a java cast, do some work to make sure* the classes are ok
		...
		if (udtTargetName != null) {
			udtTargetName.setSchemaName(getTypeId().getBaseTypeId()
					.getSchemaName());
		}
		...
		/*
		 * * If it isn't null, then we have* a cast from one JBMS type to
		 * another. So we* have to figure out if it is legit.
		 */
			...
			if (!tc.convertible(getTypeId(), forDataTypeFunction)) {
				throw StandardException.newException(
						SQLState.LANG_INVALID_CAST, sourceCTI.getSQLTypeName(),
		...
		// a non-nullable string type and the target type is a boolean, then the
		// result
		// still must be nullable because the string "unknown" casts to boolean
		// NULL.
		//
	...
	/**
	 * Get a constant representing the cast from a CHAR to another type. If this
	 * is not an "easy" cast to perform, then just return this cast node. Here's
	 * what we think is "easy": source destination ------ ----------- char
	 * boolean char date/time/ts char non-decimal numeric
	 *
	 * @param destJDBCTypeId
	 *            The destination JDBC TypeId
	 *
	...
	private ValueNode getCastFromCharConstant(int destJDBCTypeId)
		...
		case Types.BOOLEAN:
			if (cleanCharValue.equals("TRUE")) {
				...
				throw StandardException.newException(
						SQLState.LANG_FORMAT_EXCEPTION, "boolean");
			}

		case Types.DATE:
			return new UserTypeConstantNode(getDataValueFactory().getDateValue(
					cleanCharValue, false), getContextManager());

		case Types.TIMESTAMP:
			return new UserTypeConstantNode(getDataValueFactory()
					.getTimestampValue(cleanCharValue, false),
					getContextManager());

		case Types.TIME:
			return new UserTypeConstantNode(getDataValueFactory().getTimeValue(
					cleanCharValue, false), getContextManager());
		...
		case Types.BIGINT:
			try {
				...
				return getCastFromIntegralType(
						(new Double(cleanCharValue)).longValue(),
						destJDBCTypeId);
			} catch (NumberFormatException nfe) {
				String sqlName = TypeId.getBuiltInTypeId(destJDBCTypeId)
						.getSQLTypeName();
				throw StandardException.newException(
						SQLState.LANG_FORMAT_EXCEPTION, sqlName);
			}
		case Types.REAL:
			Float floatValue;
			try {
			...
			return new NumericConstantNode(TypeId.getBuiltInTypeId(Types.REAL),
					floatValue, getContextManager());
		case Types.DOUBLE:
			Double doubleValue;
			try {
	 ...
	 * type. If this is not an "easy" cast to perform, then just return this
	 * cast node. Here's what we think is "easy": source destination ------
	 * ----------- integral type non-decimal numeric integral type char
	 *
	 * @param longValue
	 *            integral type as a long to cast from
	 * @param destJDBCTypeId
	 *            The destination JDBC TypeId
	 *
	 * @return The new top of the tree (this CastNode or a new Constant)
	 ...
	 *                Thrown on error
	 */
	private ValueNode getCastFromIntegralType(long longValue, int destJDBCTypeId)
			throws StandardException {
		ValueNode retNode = this;
		...
		case Types.CHAR:
			return new CharConstantNode(Long.toString(longValue),
					getTypeServices().getMaximumWidth(), getContextManager());
		case Types.TINYINT:
			...
			return new NumericConstantNode(
					...
					ReuseFactory.getByte((byte) longValue), getContextManager());

		case Types.SMALLINT:
			if (longValue < Short.MIN_VALUE || longValue > Short.MAX_VALUE) {
				throw StandardException.newException(
						SQLState.LANG_OUTSIDE_RANGE_FOR_DATATYPE, "SHORT");
			}
			return new NumericConstantNode(
					TypeId.getBuiltInTypeId(destJDBCTypeId),
					ReuseFactory.getShort((short) longValue),
					getContextManager());
		...
		case Types.INTEGER:
			...
			return new NumericConstantNode(
					TypeId.getBuiltInTypeId(destJDBCTypeId),
					ReuseFactory.getInteger((int) longValue),
					getContextManager());

		case Types.BIGINT:
			return new NumericConstantNode(
					TypeId.getBuiltInTypeId(destJDBCTypeId),
					ReuseFactory.getLong(longValue), getContextManager());

		case Types.REAL:
			if (Math.abs(longValue) > Float.MAX_VALUE) {
				throw StandardException.newException(
						SQLState.LANG_OUTSIDE_RANGE_FOR_DATATYPE, "REAL");
			}
			return new NumericConstantNode(
					TypeId.getBuiltInTypeId(destJDBCTypeId), new Float(
							(float) longValue), getContextManager());

		case Types.DOUBLE:
			return new NumericConstantNode(
					TypeId.getBuiltInTypeId(destJDBCTypeId), new Double(
							(double) longValue), getContextManager());
		}
	 ...
	 * type. If this is not an "easy" cast to perform, then just return this
	 * cast node. Here's what we think is "easy": source destination ------
	 * ----------- non-integral type non-decimal numeric non-integral type char
	 *
	 * @param constantValue
	 *            non-integral type a a double to cast from
	 * @param destJDBCTypeId
	 *            The destination JDBC TypeId
	 *
	 * @return The new top of the tree (this CastNode or a new Constant)
	 ...
	 *                Thrown on error
	 */
	private ValueNode getCastFromNumericType(DataValueDescriptor constantValue,
			int destJDBCTypeId) throws StandardException {
		switch (destJDBCTypeId) {
		case Types.CHAR:
		...
		case Types.TINYINT:
			return new NumericConstantNode(
					TypeId.getBuiltInTypeId(destJDBCTypeId),
					Byte.valueOf(constantValue.getByte()), getContextManager());
		...
		case Types.BIGINT:
			return new NumericConstantNode(
					TypeId.getBuiltInTypeId(destJDBCTypeId),
					Long.valueOf(constantValue.getLong()), getContextManager());

		case Types.REAL:
			return new NumericConstantNode(
					TypeId.getBuiltInTypeId(destJDBCTypeId),
					Float.valueOf(NumberDataType.normalizeREAL(constantValue
							.getDouble())), getContextManager());

		case Types.DOUBLE:
			// no need to normalize here because no constant could be out of
			// range for a double
			return new NumericConstantNode(
					TypeId.getBuiltInTypeId(destJDBCTypeId),
	...
	ValueNode preprocess(int numTables, FromList outerFromList,
			...
			throws StandardException {
		castOperand = castOperand.preprocess(numTables, outerFromList,
	...
	void generateExpression(ExpressionClassBuilder acb, MethodBuilder mb)
		...
		/*
		 * HACK ALERT. When casting a parameter, there is not sourceCTI. Code
		 * generation requires one, so we simply set it to be the same as the
		 * destCTI. The user can still pass whatever type they'd like in as a
		 * parameter. They'll get an exception, as expected, if the conversion
		 * cannot be performed.
		 */
	...
	private void genDataValueConversion(ExpressionClassBuilder acb,
		 ...
		 * 
		 * Object o = sourceDVD.getObject(); targetDVD.setObjectForCast(o, o
		 * instanceof dest java type, dest java type);
		 * 
	 ...
	 * collation setting. If this method does not get called, then it means that
	 * this CAST node has been an internally generated node and we should not
	 * touch the collation of this CAST node because it has been already set
	 * correctly by the class that generated this CAST node.
	 */
	 ...
	 * 
	 * @param b
	 *            true to use function conversion rules
	 */
