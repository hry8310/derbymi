...
/**
 * This node represents either a unary IS NULL or IS NOT NULL comparison
 * operator
 *
 */

public final class IsNullNode extends UnaryComparisonOperatorNode implements
		RelationalOperator {
	private DataValueDescriptor nullValue;

	/**
	 * If {@code true}, this node represents a NOT NULL node rather than a NULL
	 * node. Note that this state is mutable, cf {@link #getNegation}.
	 */
	...
	public boolean match() {
		...
		Object lo = operand.getVal();

		if ((lo == null && methodName.equalsIgnoreCase("isNotNull"))
				|| (lo != null && methodName.equalsIgnoreCase("isNullOp"))) {
			return false;
		}
		return true;
	...
	void bindParameter() throws StandardException {
		/*
		 * * If IS [NOT] NULL has a ? operand, we assume* its type is varchar
		 * with the implementation-defined maximum length* for a varchar.* Also,
		 * for IS [NOT] NULL, it doesn't matter what is VARCHAR's* collation
		 * (since for NULL check, no collation sensitive processing* is
		 * required) and hence we will not worry about the collation setting
		 */

		operand.setType(new DataTypeDescriptor(TypeId
				.getBuiltInTypeId(Types.VARCHAR), true));
	}
	...
	/**
	 * null operators are defined on DataValueDescriptor. Overrides method in
	 * UnaryOperatorNode for code generation purposes.
	 */
	@Override
