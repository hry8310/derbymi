 ...
 * <li>public void setReferencedColumnMap(FormatableBitSet referencedColumnMap);
 * </li>
 ...
 * <li>public ExecRow getEmptyExecRow( ContextManager cm) throws
 * StandardException;</li>
 * <li>public boolean tableNameEquals(String otherSchemaName, String
 * otherTableName);</li>
 * <li>public ReferencedKeyConstraintDescriptor getPrimaryKey() throws
 * StandardException;</li>
 * <li>public void removeConglomerateDescriptor(ConglomerateDescriptor cd)
 * throws StandardException;</li>
 * <li>public void removeConstraintDescriptor(ConstraintDescriptor cd) throws
 * StandardException;</li>
 ...
 * <li>public ConglomerateDescriptor[] getConglomerateDescriptors() throws
 * StandardException;</li>
 * <li>public ConglomerateDescriptor getConglomerateDescriptor(long
 * conglomerateNumber) throws StandardException;</li>
 * <li>public ConglomerateDescriptor getConglomerateDescriptor(UUID
 * conglomerateUUID) throws StandardException;</li>
 ...
 * <li>public boolean statisticsExist(ConglomerateDescriptor cd) throws
 * StandardException;</li>
...
public class TableDescriptor extends UniqueSQLObjectDescriptor  
	...
	public static   int ISTATS_CREATE_THRESHOLD;
	public static   int ISTATS_ABSDIFF_THRESHOLD;
	public static   double ISTATS_LNDIFF_THRESHOLD;
	...
	*/
	...
	private boolean onCommitDeleteRows; // true means on commit delete rows,
										// false means on commit preserve rows
										// of temporary table.
	private boolean onRollbackDeleteRows; // true means on rollback delete rows.
	 ...
	 * The value -1 means it's uninitialized, in which case it will be
	 * initialized lazily when {@link #getHeapConglomerateId()} is called.
	 * </p>
	 *
	 * <p>
	 ...
	 * conglomerate number, and never see a partially initialized value (as was
	 * the case in DERBY-5358 because reads/writes of a long field are not
	 * guaranteed to be atomic unless the field is declared volatile).
	 * </p>
	 */
	 ...
	 * @param tableType
	 *            An integer identifier for the type of the table : declared
	 *            global temporary table
	 * @param onCommitDeleteRows
	 *            If true, on commit delete rows else on commit preserve rows of
	 *            temporary table.
	 * @param onRollbackDeleteRows
	 *            If true, on rollback, delete rows from temp tables which were
	 *            logically modified. true is the only supported value
	 */

	public TableDescriptor(DataDictionary dataDictionary, String tableName,
			SchemaDescriptor schema, int tableType, boolean onCommitDeleteRows,
	 ...
	 *            The schema descriptor for this table.
	 * @param tableType
	 *            An integer identifier for the type of the table (base table,
	 *            view, etc.)
	...
	public TableDescriptor(DataDictionary dataDictionary, String tableName,
			SchemaDescriptor schema, int tableType, char lockGranularity) {
	 ...
	 *
	 * @return A String containing the name of the schema the table lives in.
	 */
	...
	/**
	 * Gets an identifier telling what type of table this is (base table,
	 * declared global temporary table, view, etc.)
	 *
	...
	/**
	 * Gets the id for the heap conglomerate of the table. There may also be
	 * keyed conglomerates, these are stored separately in the conglomerates
	 * table.
	 *
	...
	public long getHeapConglomerateId() throws StandardException {
		...
		/*
		 * If we've already cached the heap conglomerate number, then simply
		 * return it.
		 */
	 ...
	 *
	 * @param cdl
	 *            list of columns whose positions we want to record in the bit
	 *            map
	 */
	public FormatableBitSet makeColumnMap(ColumnDescriptorList cdl) {
		FormatableBitSet result = new FormatableBitSet(
				columnDescriptorList.size() + 1);
		int count = cdl.size();
	...
	/**
	 * Gets the highest column id in the table. For now this is the same as the
	 * number of columns. However, in the future, after we implement ALTER TABLE
	 * DROP COLUMN, this correspondence won't hold any longer.
	 *
	 ...
	 *
	 * @return A boolean representing the on rollback behavior for the declared
	 *         global temporary table.
	 */
	 ...
	 *
	 * @return A boolean representing the on commit behavior for the declared
	 *         global temporary table.
	 */
	...
	/**
	 * Sets the heapConglomNumber to -1 for temporary table since the table was
	 * dropped and recreated at the commit time and hence its conglomerate id
	 * has changed. This is used for temporary table descriptors only
	 */
	public void resetHeapConglomNumber() {
		if (SanityManager.DEBUG) {
			if (tableType != TableDescriptor.GLOBAL_TEMPORARY_TABLE_TYPE) {
				SanityManager
						.THROWASSERT("tableType expected to be TableDescriptor.GLOBAL_TEMPORARY_TABLE_TYPE, not "
								+ tableType);
			}
		}
		heapConglomNumber = -1;
	 ...
	 * columnDescriptorList. This is useful for passing collation id info down
	 * to store, for instance in createConglomerate().
	 *
	 * This is only expected to get called during ddl, so object allocation is
	 * ok.
	 *
	 * @exception StandardException
	...
	public void setViewDescriptor(ViewDescriptor viewDescriptor) {
		if (SanityManager.DEBUG) {
			if (tableType != TableDescriptor.VIEW_TYPE) {
				SanityManager
						.THROWASSERT("tableType expected to be TableDescriptor.VIEW_TYPE, not "
								+ tableType);
			}
		}
	...
	/**
	 * Is this provider persistent? A stored dependency will be required if both
	 * the dependent and provider are persistent.
	 *
	 ...
	 *
	 * @param minColCount
	 *            the minimum number of ordered columns in the indexes we want
	 *            to count
	 * @param nonUniqeTrumpsColCount
	 *            if {@code true} a non-unique index will be included in the
	 *            count even if it has less than {@code minColCount} ordered
	 *            columns
	...
	/**
	 * Builds a list of all triggers which are relevant to a given statement
	 * type, given a list of updated columns.
	 *
	 * @param statementType
	 *            defined in StatementType
	 * @param changedColumnIds
	 *            array of changed columns
	 * @param relevantTriggers
	...
	public void getAllRelevantTriggers(int statementType,
			int[] changedColumnIds, TriggerDescriptorList relevantTriggers)
	...
	/**
	 * Gets all of the relevant constraints for a statement, given its statement
	 * type and its list of updated columns.
	 *
	 * @param statementType
	 *            As defined in StatementType.
	 * @param changedColumnIds
	 *            If null, all columns being changed, otherwise array of 1-based
	 *            column ids for columns being changed
	 * @param needsDeferredProcessing
	 *            IN/OUT. true if the statement already needs deferred
	 *            processing. set while evaluating this routine if a trigger or
	 *            constraint requires deferred processing
	 * @param relevantConstraints
	 *            IN/OUT. Empty list is passed in. We hang constraints on it as
	 *            we go.
	 *
	 * @exception StandardException
	 *                Thrown on error
	...
	public void getAllRelevantConstraints(int statementType,
			int[] changedColumnIds, boolean[] needsDeferredProcessing,
		...
		for (int index = 0; index < cdlSize; index++) {
			...
			/*
			 * * For each constraint, figure out if it requires deferred
			 * processing.* Note that we need to do this on constraints that
			 * don't* necessarily need to fire -- e.g. for an insert into a* a
			 * table with a self-referencing constraint, we don't* need to check
			 * the primary key constraint (assuming it* is only referenced by
			 * the self-referencing fk on the same* table), but we have to run
			 * in deferred mode nonetheless* (even though we aren't going to
			 * check the pk constraint).
			 */
			if (!needsDeferredProcessing[0]
					&& (cd instanceof ReferencedKeyConstraintDescriptor)
					&& (statementType != StatementType.UPDATE && statementType != StatementType.BULK_INSERT_REPLACE)) {
				/*
				 * For insert (bulk or regular) on a non-published table, we
				 * only need deferred mode if there is a self-referencing
			...
			if (cd.needsToFire(statementType, changedColumnIds)) {
				/*
				 * * For update, if we are updating a referenced key, then* we
				 * have to do it in deferred mode (in case we update* multiple
				 * rows).
				 */
	...
	/**
	 * Turn an array of column names into the corresponding 1-based column
	 * positions.
	 */
	 ...
	 *
	 * @param newCDL
	 *            The new constraint descriptor list for this table descriptor
	 */
	...
	public ReferencedKeyConstraintDescriptor getPrimaryKey()
			throws StandardException {
		ConstraintDescriptorList cdl = getDataDictionary()
				.getConstraintDescriptors(this);
	 ...
	 *
	 * @param newCDL
	 *            The new trigger descriptor list for this table descriptor
	 */
	...
	/**
	 * Compare the tables descriptors based on the names. Null schema names
	 * match.
	 *
	 * @param otherTableName
	 *            the other table name
	 * @param otherSchemaName
	...
	/**
	 * Remove this descriptor. Warning, removes by using object reference, not
	 * uuid.
	 *
	 * @param cd
	...
	/**
	 * Get the descriptor for a column in the table, either by the column name
	 * or by its ordinal position (column number). Returns NULL for columns that
	 * do not exist.
	 *
	 * @param columnName
	...
	/**
	 * Gets a ConglomerateDescriptor[] to loop through all the conglomerate
	 * descriptors for the table.
	 *
	 * @return A ConglomerateDescriptor[] for looping through the table's
	 *         conglomerates
	 */
	...
	/**
	 * Gets a conglomerate descriptor for the given table and conglomerate
	 * number.
	 *
	 * @param conglomerateNumber
	 *            The conglomerate number we're interested in
	 *
	 * @return A ConglomerateDescriptor describing the requested conglomerate.
	 *         Returns NULL if no such conglomerate.
	 *
	 * @exception StandardException
	 *                Thrown on failure
	 */
	public ConglomerateDescriptor getConglomerateDescriptor(
		...
		return conglomerateDescriptorList
				.getConglomerateDescriptor(conglomerateNumber);
	}

	/**
	 * Gets array of conglomerate descriptors for the given table and
	 * conglomerate number. More than one descriptors if duplicate indexes share
	 * one conglomerate.
	 *
	 * @param conglomerateNumber
	 *            The conglomerate number we're interested in
	 *
	 * @return Array of ConglomerateDescriptors with the requested conglomerate
	 *         number. Returns size 0 array if no such conglomerate.
	 *
	 * @exception StandardException
	 *                Thrown on failure
	 */
	public ConglomerateDescriptor[] getConglomerateDescriptors(
			long conglomerateNumber) throws StandardException {
		return conglomerateDescriptorList
				.getConglomerateDescriptors(conglomerateNumber);
	}

	/**
	 * Gets a conglomerate descriptor for the given table and conglomerate UUID
	 * String.
	 *
	 * @param conglomerateUUID
	 *            The UUID for the conglomerate we're interested in
	 *
	 * @return A ConglomerateDescriptor describing the requested conglomerate.
	 *         Returns NULL if no such conglomerate.
	 *
	 ...
	 */
	public ConglomerateDescriptor getConglomerateDescriptor(
			UUID conglomerateUUID) throws StandardException {
		return conglomerateDescriptorList
				.getConglomerateDescriptor(conglomerateUUID);
	}

	/**
	 * Gets array of conglomerate descriptors for the given table and
	 * conglomerate UUID. More than one descriptors if duplicate indexes share
	 * one conglomerate.
	 *
	 * @param conglomerateUUID
	 *            The conglomerate UUID we're interested in
	 *
	 * @return Array of ConglomerateDescriptors with the requested conglomerate
	 *         UUID. Returns size 0 array if no such conglomerate.
	 *
	 * @exception StandardException
	 *                Thrown on failure
	 */
	public ConglomerateDescriptor[] getConglomerateDescriptors(
			UUID conglomerateUUID) throws StandardException {
		return conglomerateDescriptorList
				.getConglomerateDescriptors(conglomerateUUID);
	}

	/**
	 * Gets an object which lists out all the index row generators on a table
	 * together with their conglomerate ids.
	 *
	 ...
	 *
	 * @return array containing the increment values of autoincrement columns.
	 *
	 */
	 ...
	 */
	public synchronized List<StatisticsDescriptor> getStatistics()
			throws StandardException {
	 ...
	 *
	 * @param tableRowCountEstimate
	 *            row count estimate for this table
	 * @throws StandardException
	 *             if obtaining index statistics fails
	 */
	public void markForIndexStatsUpdate(long tableRowCountEstimate)
		...
		for (StatisticsDescriptor sd : sdl) {
			...
			if (diff >= ISTATS_ABSDIFF_THRESHOLD) {
				double cmp = Math.abs(Math.log(indexRowCountEstimate)
	 ...
	 *            exist. cd can be null in which case user wants to know if
	 *            there are any statistics at all on the table.
	 */
	 ...
	 *
	 * @param cd
	 *            ConglomerateDescriptor (Index) whose cardinality we are
	 *            interested in.
	 * @param numKeys
	 *            Number of leading columns of the index for which cardinality
	 *            is desired.
	 */
	...
	public String getDescriptorType() {
		return (tableType == TableDescriptor.SYNONYM_TYPE) ? "Synonym"
				: "Table/View";
	}
	 ...
	 * being invalid). Always an error for a table -- should never have gotten
	 * here.
	 *
	 * @param action
