...
/**
 *
 * The ContextManager collects contexts as they are created. It maintains stacks
 * of contexts by named ids, so that the top context of a given type can be
 * returned. It also maintains a global stack so that contexts can be traversed
 * in the order they were created.
 * <p>
 * The first implementation of the context manager assumes there is only one
 * thread to worry about and that the user(s) of the class only create one
 * instance of ContextManager.
 */

public class ContextManager {
	
	public ContextManager(ContextService csf ) {
		 
	 
	}
	/**
	 * The CtxStack implement a stack on top of an ArrayList (to avoid the
	 * inherent overhead associated with java.util.Stack which is built on top
	 * of java.util.Vector, which is fully synchronized).
	 */
	private static final class CtxStack {
		...
		private final List<Context> view_ = Collections
				.unmodifiableList(stack_);
	...
	/**
	 * HashMap that holds the Context objects. The Contexts are stored with a
	 * String key.
	 * 
	...
	/**
	 * Add a Context object to the ContextManager. The object is added both to
	 * the holder list and to a stack for the specific type of Context.
	 * 
	 * @param newContext
	...
	/**
	 * Obtain the last pushed Context object of the type indicated by the
	 * contextId argument.
	 * 
	 * @param contextId
	 *            a String identifying the type of Context
	 * @return The Context object with the corresponding contextId, or null if
	 *         not found
	 */
	...
	/**
	 * Remove the last pushed Context object, regardless of type. If there are
	 * no Context objects, no action is taken.
	 */
	...
	/**
	 * Removes the specified Context object. If the specified Context object
	 * does not exist, the call will fail.
	 * 
	 * @param theContext
	...
	/**
	 * Return an unmodifiable list reference to the ArrayList backing CtxStack
	 * object for this type of Contexts. This method allows fast traversal of
	 * all Contexts on that stack. The first element in the List corresponds to
	 * the bottom of the stack. The assumption is that the Stack will not be
	...
	public final List<Context> getContextStack(String contextId) {
		...
		return cs == null ? Collections.<Context> emptyList() : cs
				.getUnmodifiableList();
	}
	 ...
	 * 
	 * @param error
	 *            the error we want to clean up
	 * @param diagActive
	 *            true if extended diagnostics should be considered, false not
	 *            interested of extended diagnostic information
	...
	public boolean cleanupOnError(Throwable error, boolean diagActive) {
		...
		if (reportError) {
			...
			if (!shutdown) {
				...
				ContextImpl lcc = (ContextImpl) getContext(ContextId.LANG_CONNECTION);
				if (lcc != null) {
		...
		/*
		 * REVISIT RESOLVE Ensure that the traversal of the stack works in all
		 * cases where contexts can pop themselves *and* contexts can pop other
		...
		forever: for (;;) {
			...
			/*
			 * Walk down the stack, calling cleanup on each context. We use the
			 * vector interface to do this.
			 */
			cleanup: for (int index = holder.size() - 1; index >= 0; index--) {
				...
				try {
					...
					// When errorSeverity greater or equals
					...
					// If Property.EXT_DIAG_SEVERITY_LEVEL is not set in JVM
					// property or
					// derby property, we will only handle threadDump
					// information and diagnosis
					// information for errorSeverity =
					...
					if (error instanceof StandardException) {
						...
						if (se.getSeverity() > ((StandardException) error)
								.getSeverity()) {
							...
							if (reportError) {
								errorStream
										.println("New exception raised during cleanup "
												+ error.getMessage());
								errorStream.flush();
							}
							continue forever;
					...
					if (reportError(se)) {
						errorStringBuilder
								.appendln("Less severe exception raised during cleanup (ignored) "
										+ se.getMessage());
					...
					if (error instanceof StandardException) {
						/*
						 * Ok, error handling raised a more severe error,
						 * restart with the more severe error A Throwable after
						 * a StandardException is always more severe.
						 */
						error = t;
						if (reportError) {
							errorStream
					...
					if (reportError) {
						errorStringBuilder
								.appendln("Equally severe exception raised during cleanup (ignored) "
										+ t.getMessage());
	...
	/**
	 * Check to see if we have been interrupted. If we have then a
	 * ShutdownException will be thrown. This will be either the one passed to
	 * interrupt or a generic one if some outside source interrupted the thread.
	 */
	...
	/**
	 * Flush the built up error string to whereever it is supposed to go, and
	...
	/**
	 * return the severity of the exception. Currently, this method does not
	 * determine a severity that is not StandardException or SQLException.
	 * 
	 * @param error
	 *            - Throwable error
	 * 
	 * @return int vendorcode/severity for the Throwable error - error/exception
	 *         to extract vendorcode/severity. For error that we can not get
	 *         severity, NO_APPLICABLE_SEVERITY will return.
	 */
	...
	/**
	 * Constructs a new instance. No CtxStacks are inserted into the hashMap as
	 * they will be allocated on demand.
	 * 
	 * @param csf
	 *            the ContextService owning this ContextManager
	 * @param stream
	 *            error stream for reporting errors
	 */
	

	  ContextService owningCsf;
	 ...
	 * ContextService.setCurrentContextManager and reset by
	 * resetCurrentContextManager. Only a single thread can be active in a
	 * ContextManager at any time, and the thread only "owns" the ContextManager
	 * while it is executing code within Derby. In the JDBC case
	 * setCurrentContextManager is called at the start of a JBDC method and
	 * resetCurrentContextManager on completion. Nesting within the same thread
	 * is supported, such as server-side JDBC calls in a Java routine or
	 * procedure. In that case the activeCount will represent the level of
	 * nesting, in some situations. <BR>
	 * 
	...
	/**
	 * Count of the number of setCurrentContextManager calls by a single thread,
	 * for nesting situations with a single active Contextmanager. If nesting is
	 * occuring with multiple different ContextManagers then this value is set
	 * to -1 and nesting is represented by entries in a stack in the ThreadLocal
	 * variable, threadContextList.
	 * 
