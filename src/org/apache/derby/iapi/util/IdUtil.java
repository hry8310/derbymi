...
/**
 * Utility class for parsing and producing string representations of ids. This
 * class supports both delimited and un-delimited ids.
 * 
 * <P>
...
public abstract class IdUtil {
	...
	/**
	 * Produce a delimited two part qualified name from two un-delimited
	 * identifiers.
	 * 
	 * @return the result.
	 */
	public static String mkQualifiedName(String id1, String id2) {
		...
		return normalToDelimited(id1) + "." + normalToDelimited(id2);
	}
	...
	/**
	 * Parse a multi-part (dot separated) SQL identifier form the String
	 * provided. Raise an excepion if the string does not contain valid SQL
	 * indentifiers. The returned String array contains the normalized form of
	 * the identifiers.
	 * 
	 * @param s
	...
	/**
	 * @param r
	 *            The multi-part identifier to be parsed
	 * @return An array of strings made by breaking the input string at its
	 *         dots, '.'.
	 * @exception StandardException
	 *                Oops
	...
	/**
	 * Parse a SQL identifier from the String provided. Raise an excepion if the
	 * string does not contain a valid SQL indentifier. The returned String
	 * contains the normalized form of the identifier.
	 * 
	 * @exception StandardException
	 *                Oops
	 */
	 ...
	 * 
	 * @param normalize
	 *            true means return ids in nomral form, false means return them
	 *            as they were entered.
	 * 
	 *            <P>
	 *            Raise an exception if the first thing in the StringReader is
	 *            not a valid id.
	 * @exception StandardException
	 *                Ooops.
	 */
	 ...
	 * non-unique, so the chosen lower case canonical form is arbitrary, e.g. we
	 * will not be able to correctly match the non-canonical:
	 * <p>
	 * 
	 * <pre>
	...
	/**
	 * Parse a regular identifier (unquoted) returning returning either the
	 * value of the identifier or a delimited identifier. Ensures that all
	 * characters in the identifer are valid for a regular identifier.
	 * 
	 * @param r
	 *            Regular identifier to parse.
	 * @param normalize
	 *            If true return the identifer converted to a single case,
	 *            otherwise return the identifier as entered.
	...
	/**
	 * Parse a delimited (quoted) identifier returning either the value of the
	 * identifier or a delimited identifier.
	 * 
	 * @param r
	 *            Quoted identifier to parse.
	 * @param normalize
	 *            If true return a delimited identifer, otherwise return the
	 *            identifier's value.
	 * @return the value of the identifer or a delimited identifier
	 * @throws IOException
	 *             Error accessing value
	...
	/**
	 * Scan a database classpath from the string provided. This returns an array
	 * with one qualified name per entry on the classpath. The constants above
	 * describe the content of the returned names. This raises an an exception
	 * if the string does not contain a valid database class path.
	 * 
	 * <PRE>
	 *       classpath := item[:item]*
	 * 	  item := id.id
			...
			try {
				...
				if (thisQName.length != 2)
					throw StandardException.newException(
							SQLState.DB_CLASS_PATH_PARSE_ERROR, input);
				...
				if (se.getMessageId().equals(SQLState.ID_PARSE_ERROR))
					throw StandardException.newException(
							SQLState.DB_CLASS_PATH_PARSE_ERROR, se, input);
				else
					throw se;
			}
				...
				throw StandardException.newException(
						SQLState.DB_CLASS_PATH_PARSE_ERROR, ioe, input);
			}
		}
		String[][] result = new String[v.size()][];
		v.copyInto(result);
	 ...
	 * This returns an array with containing the normalized forms of the
	 * identifiers.
	 * 
	 * This raises an an exception if the string does not contain a valid list
	 * of names.
	 * 
	 * @exception StandardException
	 *                Oops
	...
	/**
	 * Parse a list of comma separated SQL identifiers returning them a as
		...
		while (true) {
			...
			try {
				...
				if (se.getMessageId().equals(SQLState.ID_LIST_PARSE_ERROR))
					throw StandardException.newException(
							SQLState.ID_LIST_PARSE_ERROR, se);
				else
					throw se;
			}
				...
				throw StandardException.newException(
						SQLState.ID_LIST_PARSE_ERROR, ioe);
			}
		}
		if (v.size() == 0)
			return null;
	...
	/**
	 * Return an IdList with all the ids that in l1 and l2 or null if not ids
	 * are on both lists.
	 * 
	 * @param l1
	 *            An array of ids in normal form
	 * @param l2
	...
	/**
	 * Return an idList in external form with one id for every element of v. If
	 * v has no elements, return null.
	 * 
	 * @param normal
	 *            True means the ids in v are in normal form and false means
	 *            they are in external form.
	 */
	 ...
	 */
	public static String getUserAuthorizationId(String userName)
			throws StandardException {
		return "abc";
	}
	 ...
	 * If the user property does not exist or is set to the empty string then
	 * Property.DEFAULT_USER_NAME is returned.
	 * 
	...
	/**
	 * Return true if the normalized value of an indentifier is on the list of
	 * SQL identifiers provided.
	 * 
	 * @param id
	...
	public static boolean idOnList(String id, String list)
		...
		for (int ix = 0; ix < list_a.length; ix++)
			if (id.equals(list_a[ix]))
				return true;
		return false;
	}

	/**
	 * Delete an normal value from a list of SQL identifiers. The returned list
	 * maintains its remaining identifiers in the format they were upon entry to
	 * the call.
	 * 
	 * @param id
	 *            an id in normal form (quotes removed, upshifted if regular)
	 * @param list
	 *            a comma separated list of ids in external form (possibly
	 *            delmited or not upshifted).
	 * @return the list with the id deleted or null if the resulting list has no
	 *         ids. If 'id' is not on 'list' this returns list unchanged. If
	 *         list becomes empty after the removal null is returned.
	 * @exception StandardException
	 *                oops.
	 */
	...
	/**
	 * Append an identifier to a comma separated list of identifiers. The passed
	 * in identifier is its normal form, the list contains a list of SQL
	 * identifiers, either regular or delimited. This routine takes the easy way
	 * out and always appends a delimited identifier.
	 * 
	 ...
	 *
	 * @param roleName
	 *            role identifier to check (SQL form, has possible quoting)
	 ...
	 * 
	 * @param identifier
	 *            identifier (in case normal form) to check
	 * @param identifier_length_limit
	 *            maximum legal length
	 * @exception StandardException
	 *                normal error policy
	 */
	public static void checkIdentifierLengthLimit(String identifier,
			int identifier_length_limit) throws StandardException {
		if (identifier.length() > identifier_length_limit)
			throw StandardException.newException(
					SQLState.LANG_IDENTIFIER_TOO_LONG, identifier,
