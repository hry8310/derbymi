 ...
 * which can be extended via the grow() method. It provides additional methods
 * to manipulate BitSets. NOTE: JBitSet was driven by the (current and
 * perceived) needs of the optimizer, but placed in the util package since it is
 * not specific to query trees.. NOTE: java.util.BitSet is final, so we must
 * provide a wrapper class which includes a BitSet member in order to extend the
 * functionality. We want to make it look like JBitSet extends BitSet, so we
 * need to provide wrapper methods for all of BitSet's methods.
 *
 */
public final class JBitSet {
	 ...
	 *            The BitSet.
	 * @param size
	 *            The size of bitSet. NOTE: We need to specify the size since
	 *            the size of a BitSet is not guaranteed to be the same as
	 *            JBitSet.size().
	 */
	 ...
	 *
	 * @param jBitSet
	 *            JBitSet that we want to know if it is a subset of current
	 *            JBitSet
	 *
